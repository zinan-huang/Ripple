...
private noncomputable def bgpHoffBadPairNW (w j : ℕ) (t : ℝ) : ℝ :=
...
private noncomputable def bgpHoffOldLoserNW (w j : ℕ) (t : ℝ) : ℝ :=
  (Finset.univ.filter (fun v : UniversalLocalView =>
    v ≠ localViewU (solMUReplStaticCfg w j))).sum
      (fun v => ((bgpHeadlineSolFamNW w) w).lam v t)

private theorem bgpHoff_mix_pointwise_le_oldLoser_NW
    (w j : ℕ) {t : ℝ} (ht0 : 0 ≤ t) :
    |selectorMixTarget branchU ((bgpHeadlineSolFamNW w) w).u
        ((bgpHeadlineSolFamNW w) w).lam t haltCoordU -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤
      bgpHoffOldLoserNW w j t := by
  have hraw := selectorMixTarget_halt_to_next_of_loser_sum_sharp
    (u := ((bgpHeadlineSolFamNW w) w).u)
    (Λ := ((bgpHeadlineSolFamNW w) w).lam) (t := t)
    (c := solMUReplStaticCfg w j) (epsLam := bgpHoffOldLoserNW w j t)
    (bgpNW_lam_sum_forward w w t ht0)
    (fun v => bgpNW_lam_nonneg_forward w w v t ht0)
    (show (Finset.univ.filter (fun v : UniversalLocalView =>
      v ≠ localViewU (solMUReplStaticCfg w j))).sum
        (fun v => ((bgpHeadlineSolFamNW w) w).lam v t) ≤
      bgpHoffOldLoserNW w j t from le_rfl)
  simpa [bgpHoffOldLoserNW, solMUReplStaticCfg_step w j] using hraw

private theorem bgpHoff_mix_pointwise_le_badPair_NW
    (w j : ℕ) {t : ℝ} (ht0 : 0 ≤ t)
    (henc : selectorMUHaltEncConst (solMUReplStaticCfg w) j) :
    |selectorMixTarget branchU ((bgpHeadlineSolFamNW w) w).u
        ((bgpHeadlineSolFamNW w) w).lam t haltCoordU -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤
      bgpHoffBadPairNW w j t := by
  classical
  ...
  simpa [bgpHoffBadPairNW, sol, M, safe, oldView, newView, not_or] using hraw
...
private theorem bgpHoff_fieldCap_le_readError_add_badIntegral_NW
    (w j : ℕ) {a b : ℝ} (hab : a ≤ b) (ha0 : 0 ≤ a)
    (henc : selectorMUHaltEncConst (solMUReplStaticCfg w) j) :
    (∫ τ in a..b, bgpHeadlineHoffIntegrandNW w τ) ≤
      |((bgpHeadlineSolFamNW w) w).z a haltCoordU -
        stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| +
      2 * (∫ τ in a..b,
        selectorMUHoffGateCoeffP (p := bgpParamsNW w)
          (bgpHeadlineSolFamNW w) w τ * bgpHoffBadPairNW w j τ) := by
  let k : ℝ → ℝ := fun τ => selectorMUHoffGateCoeffP
    (p := bgpParamsNW w) (bgpHeadlineSolFamNW w) w τ
  let e : ℝ → ℝ := fun τ =>
    |selectorMixTarget branchU ((bgpHeadlineSolFamNW w) w).u
        ((bgpHeadlineSolFamNW w) w).lam τ haltCoordU -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU|
  let bad : ℝ → ℝ := bgpHoffBadPairNW w j
  have hk_cont : Continuous k := by
    simpa [k] using selectorMUHoffGateCoeffP_continuous
      (p := bgpParamsNW w) (sol := bgpHeadlineSolFamNW w) w
  have he_cont : Continuous e := by
    exact (((bgpHeadlineSolFamNW w) w).cont_mixTarget haltCoordU).sub
      continuous_const |>.abs
  have hbad_cont : Continuous bad := by
    dsimp [bad, bgpHoffBadPairNW]
    exact continuous_finsetSum _ (fun v _ =>
      ((bgpHeadlineSolFamNW w) w).cont_lam v)
  have hk0 : ∀ τ ∈ Icc a b, 0 ≤ k τ := by
    intro τ hτ
    have hτ0 := le_trans ha0 hτ.1
    simpa [k, selectorMUHoffGateCoeffP] using
      bgpNW4_zKernel_nonneg w w hτ0
  have hmono :
  ...
  have hbase' :
    ...
    simpa [bgpHeadlineHoffIntegrandNW, k, e] using hbase
...
private theorem bgpHoffBadPair_le_oldLoser_NW (w j : ℕ) {t : ℝ}
    (ht0 : 0 ≤ t) :
    bgpHoffBadPairNW w j t ≤
      (Finset.univ.filter (fun v : UniversalLocalView =>
        v ≠ localViewU (solMUReplStaticCfg w j))).sum
          (fun v => ((bgpHeadlineSolFamNW w) w).lam v t) := by
  classical
  unfold bgpHoffBadPairNW
  refine Finset.sum_le_sum_of_subset_of_nonneg ?_ ?_
  · intro v hv
    exact Finset.mem_filter.mpr
      ⟨Finset.mem_univ v, (Finset.mem_filter.mp hv).2.1⟩
  · intro v _hvold _hvbad
    exact bgpNW_lam_nonneg_forward w w v t ht0

private theorem bgpHoff_left_bad_pointwise_NW
    (w j : ℕ) {τ : ℝ}
    (hτ : τ ∈ Icc (selectorMUInterReadStart j) (selectorMUZOffStart j)) :
    selectorMUHoffGateCoeffP (p := bgpParamsNW w)
        (bgpHeadlineSolFamNW w) w τ * bgpHoffBadPairNW w j τ ≤
      bgpS4aLoserCoeffNW w j *
        Real.exp (-(250 * (bgpScaleW w : ℝ) * τ)) := by
  let S : ℝ := (bgpScaleW w : ℝ)
  let k : ℝ := selectorMUHoffGateCoeffP (p := bgpParamsNW w)
    (bgpHeadlineSolFamNW w) w τ
  let B : ℝ := bgpHoffBadPairNW w j τ
  let L : ℝ := bgpS4aLoserCoeffNW w j
  have hτ0 : 0 ≤ τ := by
  ...
  have hk0 : 0 ≤ k := by
    ...
    exact bgpNW4_zKernel_nonneg w w hτ0
  have hB0 : 0 ≤ B := by
    dsimp [B, bgpHoffBadPairNW]
    exact Finset.sum_nonneg fun v _ => bgpNW_lam_nonneg_forward w w v τ hτ0
  have hL0 : 0 ≤ L := by
    exact bgpS4aLoserCoeffNW_nonneg w j
  have hk : k ≤ Real.exp (50 * S * τ) := by
    change (bgpParamsNW w).A * ((bgpHeadlineSolFamNW w) w).α τ *
      bGateZ (bgpParamsNW w).L (((bgpHeadlineSolFamNW w) w).μ τ) τ ≤
        Real.exp (50 * (bgpScaleW w : ℝ) * τ)
    exact bgpNW_gateZ_integrand_le_halfphase w hτ0
      (bgpHeadlineLeft_sin_le_half j hτ)
  have hB : B ≤ L * Real.exp (-((bgpParamsNW w).cα * τ)) := by
    have hpair := bgpHoffBadPair_le_oldLoser_NW w j hτ0
    have hold := bgpS4a_loser_mass_timelocal_NW w j τ
      ⟨le_trans (by
...
private theorem bgpHoff_left_oldLoser_pointwise_NW
    (w j : ℕ) {τ : ℝ}
    (hτ : τ ∈ Icc (selectorMUInterReadStart j) (selectorMUZOffStart j)) :
    selectorMUHoffGateCoeffP (p := bgpParamsNW w)
        (bgpHeadlineSolFamNW w) w τ * bgpHoffOldLoserNW w j τ ≤
      bgpS4aLoserCoeffNW w j *
        Real.exp (-(250 * (bgpScaleW w : ℝ) * τ)) := by
  let S : ℝ := (bgpScaleW w : ℝ)
  let k : ℝ := selectorMUHoffGateCoeffP (p := bgpParamsNW w)
    (bgpHeadlineSolFamNW w) w τ
  let B : ℝ := bgpHoffOldLoserNW w j τ
  let L : ℝ := bgpS4aLoserCoeffNW w j
  have hτ0 : 0 ≤ τ := by
    exact le_trans (by
      unfold selectorMUInterReadStart selectorMUWriteReadTime
    ...
    have hold := bgpS4a_loser_mass_timelocal_NW w j τ
      ⟨le_trans (by
        unfold selectorMUWriteHoldTime selectorMUInterReadStart
          selectorMUWriteReadTime
        linarith [Real.pi_pos]) hτ.1, hτ.2⟩
    simpa [B, L, bgpHoffOldLoserNW] using hold
  have hprod := mul_le_mul hk hB hB0 (Real.exp_pos _).le
  calc
    selectorMUHoffGateCoeffP (p := bgpParamsNW w)
        (bgpHeadlineSolFamNW w) w τ * bgpHoffOldLoserNW w j τ = k * B := rfl
    _ ≤ Real.exp (50 * S * τ) *
        (L * Real.exp (-((bgpParamsNW w).cα * τ))) := hprod
    _ = L * Real.exp (-(250 * S * τ)) := by
...
private theorem bgpS4aLoserCoeffNW_le_exp_currency
    (w j : ℕ) :
    bgpS4aLoserCoeffNW w j ≤
      Real.exp (100 * (bgpScaleW w : ℝ) + 1000) := by
  let S : ℝ := (bgpScaleW w : ℝ)
  let card : ℝ := (Fintype.card UniversalLocalView : ℝ)
  have hL := bgpS4aLoserCoeffNW_le_card_sq_reset w j
  have hR := bgpS4ResetCoeffNW_le_one w
  have hcard := bgpNW4_card_le_exp w
  ...
  have hLR : bgpS4aLoserCoeffNW w j ≤ card ^ 2 := by
    calc
      bgpS4aLoserCoeffNW w j
          ≤ card ^ 2 * bgpS4ResetCoeffNW w := by
            dsimp only [card]
            exact hL
      _ ≤ card ^ 2 * 1 :=
      ...
      _ = card ^ 2 := by ring
  calc
    bgpS4aLoserCoeffNW w j ≤ card ^ 2 := hLR
...
private theorem bgpHoff_exp250_currency_le_budget_div_32_NW
    (w j : ℕ) {K : ℝ}
...
private theorem bgpHoff_left_badIntegral_le_budget_div_32_NW
    (w j : ℕ) :
    (∫ τ in (selectorMUInterReadStart j)..(selectorMUZOffStart j),
      selectorMUHoffGateCoeffP (p := bgpParamsNW w)
        (bgpHeadlineSolFamNW w) w τ * bgpHoffBadPairNW w j τ) ≤
  ...
  let b : ℝ := selectorMUZOffStart j
  let S : ℝ := (bgpScaleW w : ℝ)
  let K : ℝ := bgpS4aLoserCoeffNW w j
  let F : ℝ → ℝ := fun τ => selectorMUHoffGateCoeffP
    (p := bgpParamsNW w) (bgpHeadlineSolFamNW w) w τ *
      bgpHoffBadPairNW w j τ
  ...
  have hK0 : 0 ≤ K := by exact bgpS4aLoserCoeffNW_nonneg w j
  have hFcont : Continuous F := by
    apply Continuous.mul
    · simpa [F] using selectorMUHoffGateCoeffP_continuous
        (p := bgpParamsNW w) (sol := bgpHeadlineSolFamNW w) w
    · dsimp [F, bgpHoffBadPairNW]
      exact continuous_finsetSum _ (fun v _ =>
        ((bgpHeadlineSolFamNW w) w).cont_lam v)
  ...
  have hmono : (∫ τ in a..b, F τ) ≤
    ...
    exact bgpHoff_left_bad_pointwise_NW w j hτ
  have htail := integral_const_mul_exp_neg_le_left
    (r := 250 * S) (C := K) (a := a) (b := b) hr hK0 hab
  have hcurrency := bgpHoff_exp250_currency_le_budget_div_32_NW w j
    (bgpS4aLoserCoeffNW_le_exp_currency w j)
...
private theorem bgpHoff_left_oldLoserIntegral_le_budget_div_32_NW
    (w j : ℕ) :
    (∫ τ in (selectorMUInterReadStart j)..(selectorMUZOffStart j),
      selectorMUHoffGateCoeffP (p := bgpParamsNW w)
        (bgpHeadlineSolFamNW w) w τ * bgpHoffOldLoserNW w j τ) ≤
  ...
  let F : ℝ → ℝ := fun τ => selectorMUHoffGateCoeffP
    (p := bgpParamsNW w) (bgpHeadlineSolFamNW w) w τ *
      bgpHoffOldLoserNW w j τ
  have hab : a ≤ b := by simpa [a, b] using selectorMUInterReadStart_le_zOffStart j
  have hr : 0 < 250 * S := by dsimp [S]; nlinarith [bgpScaleWR_pos w]
  have hK0 : 0 ≤ K := bgpS4aLoserCoeffNW_nonneg w j
  have hFcont : Continuous F := by
    apply Continuous.mul
    · simpa [F] using selectorMUHoffGateCoeffP_continuous
        (p := bgpParamsNW w) (sol := bgpHeadlineSolFamNW w) w
    · dsimp [F, bgpHoffOldLoserNW]
      exact continuous_finsetSum _ (fun v _ =>
        ((bgpHeadlineSolFamNW w) w).cont_lam v)
  have hEcont : Continuous fun τ : ℝ => K * Real.exp (-(250 * S * τ)) := by
    fun_prop
  have hmono : (∫ τ in a..b, F τ) ≤
    ...
    apply intervalIntegral.integral_mono_on hab
      (hFcont.intervalIntegrable a b) (hEcont.intervalIntegrable a b)
    intro τ hτ
    exact bgpHoff_left_oldLoser_pointwise_NW w j hτ
  have htail := integral_const_mul_exp_neg_le_left
    (r := 250 * S) (C := K) (a := a) (b := b) hr hK0 hab
  have hcurrency := bgpHoff_exp250_currency_le_budget_div_32_NW w j
    (bgpS4aLoserCoeffNW_le_exp_currency w j)
...
entry. -/
private noncomputable def bgpS4bDeltaGFloorNW (w j : ℕ) : ℝ :=
  (Real.pi / 12) * ((1 / 4 : ℝ) ^ bgpHeadlineM *
    (((bgpWarmGainQNW w : ℚ) : ℝ) *
      Real.exp ((bgpParamsNW w).cα * bgpS2Mid j)))

private theorem bgpS4b_G_increment_ge_NW (w j : ℕ) :
    bgpS4bDeltaGFloorNW w j ≤
      ((bgpHeadlineSolFamNW w) w).G (selectorMUZOffEnd j) -
        ((bgpHeadlineSolFamNW w) w).G (bgpS2Mid j) := by
  have hab := bgpS2_mid_le_zOffEnd j
  have ha0 := bgpS2_mid_nonneg j
  rw [bgpNW4_G_sub_eq w w ha0 hab]
  have hconst : bgpS4bDeltaGFloorNW w j =
      ∫ _s in (bgpS2Mid j)..(selectorMUZOffEnd j),
        ((1 / 4 : ℝ) ^ bgpHeadlineM *
          (((bgpWarmGainQNW w : ℚ) : ℝ) *
            Real.exp ((bgpParamsNW w).cα * bgpS2Mid j))) := by
    rw [intervalIntegral.integral_const, smul_eq_mul]
    unfold bgpS4bDeltaGFloorNW bgpS2Mid selectorMUZOffEnd
    ring
  rw [hconst]
  refine intervalIntegral.integral_mono_on hab intervalIntegrable_const
    ((bgpNW4cg_cont w).intervalIntegrable _ _) ?_
  intro s hs
  have hsin := bgpS2_sin_ge_neg_half j
    (le_trans (bgpS2_recStart_le_mid j) hs.1) hs.2
  have hpow : (1 / 4 : ℝ) ^ bgpHeadlineM ≤
      ((1 + Real.sin s) / 2) ^ bgpHeadlineM :=
    pow_le_pow_left₀ (by norm_num) (by linarith) _
  have hexp : Real.exp ((bgpParamsNW w).cα * bgpS2Mid j) ≤
      Real.exp ((bgpParamsNW w).cα * s) := by
    apply Real.exp_le_exp.mpr
    exact mul_le_mul_of_nonneg_left hs.1 (bgpParamsNW_cα_pos w).le
  unfold bgpNW4cg
  exact mul_le_mul hpow
    (mul_le_mul_of_nonneg_left hexp (bgpWarmGainQNW_nonneg_real w))
    (mul_nonneg (bgpWarmGainQNW_nonneg_real w) (Real.exp_pos _).le)
    (pow_nonneg (by nlinarith [Real.neg_one_le_sin s]) _)

private noncomputable def bgpS4bInitialCoeffNW (w j : ℕ) : ℝ :=
  Real.exp (-(15 / 16 : ℝ) * bgpS4bDeltaGFloorNW w j +
    (bgpParamsNW w).cα * selectorMUZOffEnd j)

private theorem bgpS4bInitialCoeffNW_nonneg (w j : ℕ) :
    0 ≤ bgpS4bInitialCoeffNW w j := (Real.exp_pos _).le

private theorem bgpS4bInitialCoeffNW_le_one (w j : ℕ) :
    bgpS4bInitialCoeffNW w j ≤ 1 := by
  let S : ℝ := (bgpScaleW w : ℝ)
  let g : ℝ := ((bgpWarmGainQNW w : ℚ) : ℝ)
  let c : ℝ := (bgpParamsNW w).cα
  let a : ℝ := bgpS2Mid j
  let z : ℝ := selectorMUZOffEnd j
  let q : ℝ := (Real.pi / 12) * (1 / 4 : ℝ) ^ bgpHeadlineM
  have hS : (5184 : ℝ) ≤ S := by dsimp only [S]; exact bgpNW4_S_ge w
  have hS0 : 0 < S := by dsimp only [S]; exact bgpScaleWR_pos w
  have hg0 : 0 < g := by dsimp only [g]; exact bgpWarmGainQNW_pos_real w
  have hc0 : 0 < c := by dsimp only [c]; exact bgpParamsNW_cα_pos w
  have ha0 : 0 ≤ a := by dsimp only [a]; exact bgpS2_mid_nonneg j
  have hq0 : 0 ≤ q := by
    dsimp only [q]
    positivity
...
private theorem bgpS4bInitialCoeffNW_le_reset (w j : ℕ) :
  ...
  let q : ℝ := (Real.pi / 12) * (1 / 4 : ℝ) ^ bgpHeadlineM
  let A : ℝ := (15 / 16 : ℝ) * q * g
  have hg : 0 < g := by dsimp only [g]; exact bgpWarmGainQNW_pos_real w
  ...
  have hgbase : (1734736490 : ℝ) * (5184 : ℝ) ^ 6 ≤ g := by
    have hS6 : (5184 : ℝ) ^ 6 ≤ (bgpScaleW w : ℝ) ^ 6 :=
      pow_le_pow_left₀ (by norm_num) (bgpNW4_S_ge w) 6
  ...
  have hfloor : A * Real.exp (c * a) =
      (15 / 16 : ℝ) * bgpS4bDeltaGFloorNW w j := by
    dsimp only [A, q, g, c, a]
    unfold bgpS4bDeltaGFloorNW
    ring
  have hinit_exp : bgpS4bInitialCoeffNW w j ≤ Real.exp (-g) := by
    unfold bgpS4bInitialCoeffNW
    apply Real.exp_le_exp.mpr
    rw [show -(15 / 16 : ℝ) * bgpS4bDeltaGFloorNW w j =
      -((15 / 16 : ℝ) * bgpS4bDeltaGFloorNW w j) by ring,
      ← hfloor]
    dsimp only [c, z]
    linarith
  ...
  have hinv : Real.exp (-g) ≤ 1 / g := by
    ...
    nlinarith
  calc
    bgpS4bInitialCoeffNW w j ≤ Real.exp (-g) := hinit_exp
    _ ≤ 1 / g := hinv
    _ ≤ (2 : ℝ) ^ (2 * bgpHeadlineM + 3) / g := by
      exact div_le_div_of_nonneg_right
        (by norm_num [bgpHeadlineM]) hg.le
    _ = bgpS4bResetCoeffNW w := by rfl

private theorem bgpS4b_homogeneous_timelocal_NW
    (w j : ℕ) {gap t : ℝ}
    (hgap : (15 : ℝ) / 16 ≤ gap)
  ...
  have haz : a ≤ z := bgpS2_mid_le_zOffEnd j
  have hz0 : 0 ≤ z := le_trans ha0 haz
  have hzt : z ≤ t := ht.1
  have hΔ := bgpS4b_G_increment_ge_NW w j
  have hΔ0 : 0 ≤ bgpS4bDeltaGFloorNW w j := by
    unfold bgpS4bDeltaGFloorNW
    exact mul_nonneg (by positivity)
      (mul_nonneg (by positivity)
        (mul_nonneg (bgpWarmGainQNW_nonneg_real w) (Real.exp_pos _).le))
  have hfirst : Real.exp (-gap * (sol.G z - sol.G a)) ≤
      Real.exp (-(15 / 16 : ℝ) * bgpS4bDeltaGFloorNW w j) := by
    apply Real.exp_le_exp.mpr
    have hG0 : 0 ≤ sol.G z - sol.G a := hΔ0.trans (by simpa [sol, z, a] using hΔ)
    have hΔ' : bgpS4bDeltaGFloorNW w j ≤ sol.G z - sol.G a := by
  ...
  have hpost : Real.exp (-gap * (sol.G t - sol.G z)) ≤
    ...
    have h := exp_neg_G_sub_le_exp_mul_exp_neg_of_rate_le
      ...
      (fun s hs => by
        ...
        simpa [sol, cg] using bgpNW4_G_hasDeriv w w hs0)
      (bgpNW4cg_cont w)
      (fun s hs => by
        have hswin : s ∈ Icc (bgpS2Mid j) (selectorMUNextWriteStart j) :=
          ⟨le_trans haz hs.1, le_trans hs.2 ht.2⟩
        simpa [c, cg] using bgpS4b_gap_mul_cg_ge_calpha_NW w j hgap hswin)
  ...
  calc
    Real.exp (-gap * (sol.G t - sol.G a)) =
      ...
      congr 1
      ring
    _ ≤ Real.exp (-(15 / 16 : ℝ) * bgpS4bDeltaGFloorNW w j) *
          (Real.exp (c * z) * Real.exp (-(c * t))) :=
      mul_le_mul hfirst hpost (Real.exp_pos _).le (Real.exp_pos _).le
    _ = bgpS4bInitialCoeffNW w j * Real.exp (-(c * t)) := by
      unfold bgpS4bInitialCoeffNW
      rw [Real.exp_add]
      ring
    _ = bgpS4bInitialCoeffNW w j *
          Real.exp (-((bgpParamsNW w).cα * t)) := rfl

private noncomputable def bgpS4bLoserCoeffNW (w j : ℕ) : ℝ :=
  ((Fintype.card UniversalLocalView : ℝ) - 1) *
    ((Fintype.card UniversalLocalView : ℝ) * bgpS4bInitialCoeffNW w j +
      bgpS4bResetCoeffNW w)

private theorem bgpS4bLoserCoeffNW_nonneg (w j : ℕ) :
    0 ≤ bgpS4bLoserCoeffNW w j := by
  ...
  unfold bgpS4bLoserCoeffNW
  exact mul_nonneg (sub_nonneg.mpr hcard)
    (add_nonneg
      (mul_nonneg (by positivity) (bgpS4bInitialCoeffNW_nonneg w j))
      (bgpS4bResetCoeffNW_nonneg w))

private theorem bgpS4bLoserCoeffNW_le_card_sq_reset (w j : ℕ) :
    bgpS4bLoserCoeffNW w j ≤
      (Fintype.card UniversalLocalView : ℝ) ^ 2 * bgpS4bResetCoeffNW w := by
  let card : ℝ := (Fintype.card UniversalLocalView : ℝ)
  ...
  have hinit := bgpS4bInitialCoeffNW_le_reset w j
  unfold bgpS4bLoserCoeffNW
  dsimp [card] at hcard ⊢
  have hinside :
      (Fintype.card UniversalLocalView : ℝ) * bgpS4bInitialCoeffNW w j +
  ...
  calc
    ...
    _ ≤ (Fintype.card UniversalLocalView : ℝ) ^ 2 *
          bgpS4bResetCoeffNW w := by nlinarith

private theorem bgpS4b_loser_mass_timelocal_NW (w j : ℕ) :
  ...
  let sol := (bgpHeadlineSolFamNW w) w
  let a := bgpS2Mid j
  let gap := selectorReplicatorGapVal bgpHeadlineEta bgpHeadlineEta_pos
  let Kt := ∫ s in a..t,
  ...
  have hgap : (15 : ℝ) / 16 ≤ gap := by
    simpa [gap, bgpRecoveryGap] using bgpRecoveryGap_ge_fifteen_sixteen w (j + 1)
  have hraw := bgpS4b_loser_mass_exact_NW w j t
    ⟨le_trans (bgpS2_mid_le_zOffEnd j) ht.1, ht.2⟩
  rw [epsLamSettled_card_inv] at hraw
  have hX : X ≤ bgpS4bInitialCoeffNW w j *
      Real.exp (-((bgpParamsNW w).cα * t)) := by
    simpa [X, sol, a, gap] using bgpS4b_homogeneous_timelocal_NW w j hgap ht
  have hKX : Kt * X ≤ bgpS4bResetCoeffNW w *
      Real.exp (-((bgpParamsNW w).cα * t)) := by
    simpa [Kt, X, sol, a, gap, neg_mul] using
      bgpS4b_reset_duhamel_timelocal_NW w j hgap
        ⟨le_trans (bgpS2_mid_le_zOffEnd j) ht.1, ht.2⟩
  ...
  calc
    ...
    _ = bgpS4bLoserCoeffNW w j *
          Real.exp (-((bgpParamsNW w).cα * t)) := by
      unfold bgpS4bLoserCoeffNW
      ring

private theorem bgpHoffBadPair_le_newLoser_NW (w j : ℕ) {t : ℝ}
    (ht0 : 0 ≤ t) :
    bgpHoffBadPairNW w j t ≤
      (Finset.univ.filter (fun v : UniversalLocalView =>
        v ≠ localViewU (solMUReplStaticCfg w (j + 1)))).sum
...
private theorem bgpHoff_rightCross_sin_le_sqrt2 (j : ℕ) {t : ℝ}
...
private theorem bgpHoff_rightCross_gate_le_exp200_NW
    (w j : ℕ) {τ : ℝ}
  ...
  have hsin := bgpHoff_rightCross_sin_le_sqrt2 j hτ
  ...
  have hr : (1 / 10 : ℝ) ≤ ((1 - Real.sin τ) / 2) ^ 1 := by
    rw [pow_one]
    linarith
  have hα := bgpHeadlineSolFamNW_alpha w w hτ0
  have hμ := bgpHeadlineSolFamNW_mu w w hτ0
...
private theorem bgpHoff_right_bad_pointwise_NW
    (w j : ℕ) {τ : ℝ}
  ...
  let k : ℝ := selectorMUHoffGateCoeffP (p := bgpParamsNW w)
    (bgpHeadlineSolFamNW w) w τ
  let B : ℝ := bgpHoffBadPairNW w j τ
  let L : ℝ := bgpS4bLoserCoeffNW w j
  have hτ0 : 0 ≤ τ := by
  ...
  have hk : k ≤ Real.exp (200 * S * τ) := by
    exact bgpHoff_rightCross_gate_le_exp200_NW w j hτ
  have hB : B ≤ L * Real.exp (-((bgpParamsNW w).cα * τ)) := by
    exact (bgpHoffBadPair_le_newLoser_NW w j hτ0).trans
      (bgpS4b_loser_mass_timelocal_NW w j τ ⟨hτ.1, by
        calc
...
private theorem bgpS4bLoserCoeffNW_le_exp_currency (w j : ℕ) :
    bgpS4bLoserCoeffNW w j ≤
      Real.exp (100 * (bgpScaleW w : ℝ) + 1000) := by
  let S : ℝ := (bgpScaleW w : ℝ)
  let card : ℝ := (Fintype.card UniversalLocalView : ℝ)
  have hcard : (1 : ℝ) ≤ card := by
    ...
    exact_mod_cast Fintype.card_pos_iff.mpr
      (⟨defaultLocalViewU⟩ : Nonempty UniversalLocalView)
  have hI := bgpS4bInitialCoeffNW_le_one w j
  have hR := bgpS4bResetCoeffNW_le_one w
  have hL : bgpS4bLoserCoeffNW w j ≤ 2 * card ^ 2 := by
    unfold bgpS4bLoserCoeffNW
    have hin : card * bgpS4bInitialCoeffNW w j + bgpS4bResetCoeffNW w ≤
        card + 1 := by
      nlinarith [mul_le_mul_of_nonneg_left hI (by linarith : 0 ≤ card)]
    have hm1 : 0 ≤ card - 1 := by linarith
    calc
      (card - 1) * (card * bgpS4bInitialCoeffNW w j + bgpS4bResetCoeffNW w)
          ≤ (card - 1) * (card + 1) := mul_le_mul_of_nonneg_left hin hm1
      _ ≤ 2 * card ^ 2 := by nlinarith [sq_nonneg card]
  have hcard' : card ≤ Real.exp (6 * S) := by
    dsimp only [card, S]
    exact bgpNW4_card_le_exp w
  have hcard2 : card ^ 2 ≤ Real.exp (12 * S) := by
  ...
  calc
    bgpS4bLoserCoeffNW w j ≤ 2 * card ^ 2 := hL
...
private theorem bgpHoff_right_badIntegral_le_budget_div_32_NW
    (w j : ℕ) :
  ...
  let S : ℝ := (bgpScaleW w : ℝ)
  let K : ℝ := bgpS4bLoserCoeffNW w j
  let F : ℝ → ℝ := fun τ => selectorMUHoffGateCoeffP
    (p := bgpParamsNW w) (bgpHeadlineSolFamNW w) w τ *
      bgpHoffBadPairNW w j τ
  have hab : a ≤ b := by
  ...
  have hK0 : 0 ≤ K := bgpS4bLoserCoeffNW_nonneg w j
  have hFcont : Continuous F := by
    apply Continuous.mul
    · simpa [F] using selectorMUHoffGateCoeffP_continuous
        (p := bgpParamsNW w) (sol := bgpHeadlineSolFamNW w) w
    · dsimp [F, bgpHoffBadPairNW]
      exact continuous_finsetSum _ (fun v _ => ((bgpHeadlineSolFamNW w) w).cont_lam v)
  ...
  have hmono : (∫ τ in a..b, F τ) ≤
    ...
    change selectorMUHoffGateCoeffP (p := bgpParamsNW w)
      (bgpHeadlineSolFamNW w) w τ * bgpHoffBadPairNW w j τ ≤
        bgpS4bLoserCoeffNW w j * Real.exp (-(100 * (bgpScaleW w : ℝ) * τ))
    exact bgpHoff_right_bad_pointwise_NW w j hτ
  have htail := integral_const_mul_exp_neg_le_left
    (r := 100 * S) (C := K) (a := a) (b := b) hr hK0 hab
  have hK : K ≤ Real.exp (100 * S + 1000) := by
    dsimp only [K, S]
    exact bgpS4bLoserCoeffNW_le_exp_currency w j
  let T : ℝ := 2 * Real.pi * (j : ℝ) + Real.pi
...
private noncomputable def bgpS4bForcingCoeffNW
    (w j : ℕ) (_v : UniversalLocalView) : ℝ :=
  ((bgpWarmGainQNW w : ℚ) : ℝ) * (bgpS4bLoserCoeffNW w j) ^ 2

private theorem bgpS4bForcingCoeffNW_nonneg
    (w j : ℕ) (v : UniversalLocalView) :
    0 ≤ bgpS4bForcingCoeffNW w j v := by
  unfold bgpS4bForcingCoeffNW
  exact mul_nonneg (bgpWarmGainQNW_nonneg_real w) (sq_nonneg _)

private theorem bgpS4b_forcing_pointwise_exp_NW
    (w j : ℕ) (v : UniversalLocalView)
    (hv : v ≠ localViewU (solMUReplStaticCfg w (j + 1)))
    (τ : ℝ)
    (hτ : τ ∈ Icc (selectorMUEarlyWriteSubStart (j + 1))
      (selectorMUNextWriteStart j)) :
    |selectorMU_activeCgP (p := bgpParamsNW w) bgpHeadlineM
          (bgpWarmGainQNW w) τ *
        (∑ x : UniversalLocalView,
          ((bgpHeadlineSolFamNW w) w).lam x τ *
            (universalPval bgpHeadlineEta bgpHeadlineEta_pos
                (localViewU (solMUReplStaticCfg w (j + 1)))
                (((bgpHeadlineSolFamNW w) w).u τ) -
              universalPval bgpHeadlineEta bgpHeadlineEta_pos x
                (((bgpHeadlineSolFamNW w) w).u τ))) *
        ((bgpHeadlineSolFamNW w) w).lam v τ| ≤
      bgpS4bForcingCoeffNW w j v *
  ...
  let gapf : UniversalLocalView → ℝ := fun x =>
    universalPval bgpHeadlineEta bgpHeadlineEta_pos cview (sol.u τ) -
      universalPval bgpHeadlineEta bgpHeadlineEta_pos x (sol.u τ)
  ...
  have hlam0 : ∀ x : UniversalLocalView, 0 ≤ lam x := fun x =>
    bgpNW_lam_nonneg_forward w w x τ hτ0
  have hcg0 : 0 ≤ cg := by
  ...
  have hgap0 : ∀ x : UniversalLocalView, 0 ≤ gapf x := by
    ...
    by_cases hx : x = cview
    · subst x; simp [gapf]
    · have hfloor := bgpS4b_gapFloor_NW w j x
  ...
  have hgap_upper : ∀ x : UniversalLocalView, x ≠ cview → gapf x ≤ 1 := by
    intro x hx
    have hut := bgpS4b_right_edge_tube_persist_NW w j τ hτfull
    exact bgpHeadline_universalPval_sub_le_one_of_utube
      (fun i => hut i) cview x
  have hsum := bgp_filtered_forcing_pointwise_le_delta_loser_square
  ...
  have hB := bgpS4b_loser_mass_timelocal_NW w j τ hτfull
  have hB' : Bmass ≤ bgpS4bLoserCoeffNW w j *
      Real.exp (-((bgpParamsNW w).cα * τ)) := by
      ...
      calc
        (((bgpWarmGainQNW w : ℚ) : ℝ) *
            ...
            (bgpS4bLoserCoeffNW w j *
              Real.exp (-((bgpParamsNW w).cα * τ))) ^ 2 =
          (((bgpWarmGainQNW w : ℚ) : ℝ) *
              (bgpS4bLoserCoeffNW w j) ^ 2) *
            (Real.exp ((bgpParamsNW w).cα * τ) *
              Real.exp (-((bgpParamsNW w).cα * τ))) *
            Real.exp (-((bgpParamsNW w).cα * τ)) := by ring
        _ = _ := by rw [hexpc]; ring

private theorem bgpS4b_forcing_coeff_sum_le (w j : ℕ) :
    (Finset.univ.filter (fun v : UniversalLocalView =>
      v ≠ localViewU (solMUReplStaticCfg w j) ∧
        v ≠ localViewU (solMUReplStaticCfg w (j + 1)))).sum
        (fun v => bgpS4bForcingCoeffNW w j v) ≤
  ...
  have hcard0 : 0 < card := by
    ...
    exact_mod_cast Fintype.card_pos_iff.mpr
      (⟨defaultLocalViewU⟩ : Nonempty UniversalLocalView)
  have hg : 0 < g := by dsimp [g]; exact bgpWarmGainQNW_pos_real w
  have hcardg : card ≤ g := by
    simpa [card, g] using bgpS4_card_le_gain_NW w
  have hL := bgpS4bLoserCoeffNW_le_card_sq_reset w j
  have hL0 := bgpS4bLoserCoeffNW_nonneg w j
  have hLsq : (bgpS4bLoserCoeffNW w j) ^ 2 ≤
      (card ^ 2 * bgpS4bResetCoeffNW w) ^ 2 :=
    pow_le_pow_left₀ hL0 (by simpa [card] using hL) 2
  have hK : ∀ v : UniversalLocalView,
      bgpS4bForcingCoeffNW w j v ≤ (2 : ℝ) ^ 86 * card ^ 3 := by
    intro v
    have hmul := mul_le_mul_of_nonneg_left hLsq hg.le
    have hcancel :
        g * (card ^ 2 * bgpS4bResetCoeffNW w) ^ 2 =
          (2 : ℝ) ^ 86 * card ^ 4 / g := by
      rw [bgpS4bResetCoeffNW]
      norm_num [bgpHeadlineM]
      dsimp only [g]
      field_simp [bgpWarmGainQNW_pos_real w |>.ne']
      ring
  ...
  calc
    bad.sum (fun v => bgpS4bForcingCoeffNW w j v)
...
private theorem bgpS4b_forcing_exp_budget_NW (w j : ℕ) :
  ...
  let Ksum : ℝ :=
    (Finset.univ.filter (fun v : UniversalLocalView =>
      v ≠ localViewU (solMUReplStaticCfg w j) ∧
        v ≠ localViewU (solMUReplStaticCfg w (j + 1)))).sum
      (fun v => bgpS4bForcingCoeffNW w j v)
  have hS : (5184 : ℝ) ≤ S := by dsimp [S]; exact bgpNW4_S_ge w
  have hS0 : 0 < S := by dsimp [S]; exact bgpScaleWR_pos w
  have hr : 1 ≤ r := by dsimp [r]; nlinarith
  have hK0 : 0 ≤ Ksum := by
    dsimp [Ksum]
    exact Finset.sum_nonneg (fun v _ => bgpS4bForcingCoeffNW_nonneg w j v)
  have hK : Ksum ≤ (2 : ℝ) ^ 86 * card ^ 4 := by
    dsimp only [Ksum, card]
    exact bgpS4b_forcing_coeff_sum_le w j
  ...
  refine bgpS13_three_rate_le_budget_div_32 j ?_
  change 2 * (Ksum / r * Real.exp (-(r * a))) ≤ _
  have hpos := Real.exp_pos (-(200 * Real.pi * (2 * (j : ℝ) + 1)))
  nlinarith

private theorem bgpS4b_forcingMass_le_NW (w j : ℕ) :
        ...
        ((bgpHeadlineSolFamNW w) w).lam x τ *
          (universalPval bgpHeadlineEta bgpHeadlineEta_pos
            ...
            universalPval bgpHeadlineEta bgpHeadlineEta_pos x
              (((bgpHeadlineSolFamNW w) w).u τ))) *
      ((bgpHeadlineSolFamNW w) w).lam v τ
  let K : UniversalLocalView → ℝ := fun v => bgpS4bForcingCoeffNW w j v
  have hab : a ≤ b := by
    dsimp [a, b]
    unfold selectorMUEarlyWriteSubStart selectorMUNextWriteStart
      selectorMUWriteHoldTime
  ...
  have hK0 : ∀ v ∈ bad, 0 ≤ K v := by
    intro v _; exact bgpS4bForcingCoeffNW_nonneg w j v
  have hF_cont : ∀ v ∈ bad, ContinuousOn (F v) (Icc a b) := by
    intro v _
    have hcg := selectorMU_activeCg_continuousP
      (p := bgpParamsNW w) bgpHeadlineM (bgpWarmGainQNW w)
    have hmean : Continuous fun τ : ℝ =>
      ...
      intro x _
      exact (((bgpHeadlineSolFamNW w) w).cont_lam x).mul
        ((universalPval_continuous_of_cont_u bgpHeadlineEta
            bgpHeadlineEta_pos _
            (fun i => ((bgpHeadlineSolFamNW w) w).cont_u i)).sub
          (universalPval_continuous_of_cont_u bgpHeadlineEta
            bgpHeadlineEta_pos x
            (fun i => ((bgpHeadlineSolFamNW w) w).cont_u i)))
    simpa [F] using
      ((hcg.mul hmean).mul (((bgpHeadlineSolFamNW w) w).cont_lam v)).continuousOn
  have hpoint : ∀ v ∈ bad, ∀ τ ∈ Icc a b,
      |F v τ| ≤ K v * Real.exp (-(r * τ)) := by
    intro v hv τ hτ
    have hvNew := (Finset.mem_filter.mp hv).2.2
    have hp := bgpS4b_forcing_pointwise_exp_NW w j v hvNew τ
  ...
  have hbudget : 2 * (((bad.sum K) / r) * Real.exp (-(r * a))) ≤
      selectorMUHoffEdgeBudget3992 j / 32 := by
    simpa only [bad, K, r, a] using bgpS4b_forcing_exp_budget_NW w j
...
private theorem bgpNW_lam_hasDerivAt_gapTracking
    (w : ℕ) {τ : ℝ} (hτ0 : 0 ≤ τ)
    (c v : UniversalLocalView) :
    HasDerivAt (((bgpHeadlineSolFamNW w) w).lam v)
      (selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa τ *
          (Fintype.card UniversalLocalView : ℝ)⁻¹ -
        selectorMU_activeSinkP bgpHeadlineEta bgpHeadlineEta_pos
          ...
          (∑ x : UniversalLocalView,
            ((bgpHeadlineSolFamNW w) w).lam x τ *
              (universalPval bgpHeadlineEta bgpHeadlineEta_pos c
                ...
                universalPval bgpHeadlineEta bgpHeadlineEta_pos x
                  (((bgpHeadlineSolFamNW w) w).u τ))) *
          ((bgpHeadlineSolFamNW w) w).lam v τ) τ := by
  classical
  let sol := (bgpHeadlineSolFamNW w) w
  let cr : ℝ := selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa τ
  let cg : ℝ := selectorMU_activeCgP (p := bgpParamsNW w)
    bgpHeadlineM (bgpWarmGainQNW w) τ
  let P : UniversalLocalView → ℝ := fun u =>
    universalPval bgpHeadlineEta bgpHeadlineEta_pos u (sol.u τ)
  ...
  have hrewrite :=
    SelectorDynSol.replicatorLamRHS_eq_gapTrackingResidual_add_meanGap
      ...
      (cr := cr) (cg := cg) (c := c) (v := v)
      (by simpa [lam, sol] using bgpNW_lam_sum_forward w w τ hτ0)
...
private theorem bgpNW_mixTargetDerivRHS_halt_centered
    (w : ℕ) {τ : ℝ} (hτ0 : 0 ≤ τ) (c : UniversalLocalView) :
    SelectorReplicatorDynSol.mixTargetDerivRHS
        ((bgpHeadlineSolFamNW w) w) τ haltCoordU =
      ∑ v : UniversalLocalView,
        (((selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa τ *
              (Fintype.card UniversalLocalView : ℝ)⁻¹ -
            selectorMU_activeSinkP bgpHeadlineEta bgpHeadlineEta_pos
          ...
          (BranchData.evalBranch (branchU v)
              (((bgpHeadlineSolFamNW w) w).u τ) haltCoordU -
            BranchData.evalBranch (branchU c)
              (((bgpHeadlineSolFamNW w) w).u τ) haltCoordU))) := by
  classical
  haveI : Nonempty UniversalLocalView := ⟨c⟩
  have hcenter := SelectorReplicatorDynSol.mixTargetDerivRHS_eq_centered
    (sol := ((bgpHeadlineSolFamNW w) w)) τ haltCoordU c
    (bgpNW_lam_sum_forward w w τ hτ0)
  have hraw : ∀ v : UniversalLocalView,
      HasDerivAt (((bgpHeadlineSolFamNW w) w).lam v)
        (((((1 + Real.cos τ) / 2) ^ bgpHeadlineM *
              (bgpHeadlineKappa : ℝ)) *
            ...
            ((bgpHeadlineSolFamNW w) w).lam v τ *
              (universalPval bgpHeadlineEta bgpHeadlineEta_pos v
                ...
                ∑ x : UniversalLocalView,
                  ((bgpHeadlineSolFamNW w) w).lam x τ *
                    universalPval bgpHeadlineEta bgpHeadlineEta_pos x
                      (((bgpHeadlineSolFamNW w) w).u τ)))) τ := by
    intro v
    simpa [selectorSchedule] using
      ((bgpHeadlineSolFamNW w) w).lam_hasDeriv v τ
        (by simpa [selectorSchedule] using hτ0)
  have hgap := bgpNW_lam_hasDerivAt_gapTracking w hτ0 c
  rw [hcenter]
...
private theorem bgpS4b_absRHS_le_badPair_sum_NW
    (w j : ℕ)
    (henc : selectorMUHaltEncConst (solMUReplStaticCfg w) j)
    {τ : ℝ} (hτ0 : 0 ≤ τ) :
    |SelectorReplicatorDynSol.mixTargetDerivRHS
              ...
              (∑ x : UniversalLocalView,
                ((bgpHeadlineSolFamNW w) w).lam x τ *
                  (universalPval bgpHeadlineEta bgpHeadlineEta_pos
                    ...
                    universalPval bgpHeadlineEta bgpHeadlineEta_pos x
                      (((bgpHeadlineSolFamNW w) w).u τ))) *
              ((bgpHeadlineSolFamNW w) w).lam v τ|) := by
  classical
  ...
  have hkill : ∀ v : UniversalLocalView,
      ...
      BranchData.evalBranch (branchU v)
          (((bgpHeadlineSolFamNW w) w).u τ) haltCoordU -
        BranchData.evalBranch (branchU c)
          (((bgpHeadlineSolFamNW w) w).u τ) haltCoordU = 0 := by
    intro v hv
    rcases hv with rfl | rfl
    · exact sub_eq_zero_of_eq hBeq
    · exact sub_self _
  rw [bgpNW_mixTargetDerivRHS_halt_centered w hτ0 c]
  calc
    |∑ v : UniversalLocalView,
        ((selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa τ *
              (Fintype.card UniversalLocalView : ℝ)⁻¹ -
    ...
    _ = (Finset.univ.filter (fun v : UniversalLocalView =>
          ...
          (fun v => |(selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa τ *
                (Fintype.card UniversalLocalView : ℝ)⁻¹ -
            selectorMU_activeSinkP bgpHeadlineEta bgpHeadlineEta_pos
                (bgpHeadlineSolFamNW w) w c v τ *
              ((bgpHeadlineSolFamNW w) w).lam v τ +
...
private noncomputable def bgpS4bForceNW
...
private noncomputable def bgpS4bDefectNW
    (w j : ℕ) (v : UniversalLocalView) (τ : ℝ) : ℝ :=
  selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa τ *
      (Fintype.card UniversalLocalView : ℝ)⁻¹ -
    selectorMU_activeSinkP bgpHeadlineEta bgpHeadlineEta_pos
        ...
        (localViewU (solMUReplStaticCfg w (j + 1))) v τ *
      ((bgpHeadlineSolFamNW w) w).lam v τ +
    bgpS4bForceNW w j v τ

private theorem bgpS4b_perView_TV_le_NW
    (w j : ℕ) (v : UniversalLocalView)
    (hv : v ≠ localViewU (solMUReplStaticCfg w (j + 1))) :
    (∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
        (selectorMUNextWriteStart j), |bgpS4bDefectNW w j v τ|) ≤
      |selectorMU_activeQSSP bgpHeadlineEta bgpHeadlineEta_pos
          (bgpHeadlineSolFamNW w) w
          (localViewU (solMUReplStaticCfg w (j + 1))) v
          (selectorMUEarlyWriteSubStart (j + 1)) -
        ((bgpHeadlineSolFamNW w) w).lam v
          (selectorMUEarlyWriteSubStart (j + 1))| +
      (∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
          (selectorMUNextWriteStart j),
        |selectorMU_activeQSSDerivRHSP bgpHeadlineEta
          bgpHeadlineEta_pos (bgpHeadlineSolFamNW w) w
          (localViewU (solMUReplStaticCfg w (j + 1))) v τ|) +
      2 * (∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
          (selectorMUNextWriteStart j), |bgpS4bForceNW w j v τ|) := by
  classical
  let a : ℝ := selectorMUEarlyWriteSubStart (j + 1)
  let b : ℝ := selectorMUNextWriteStart j
  let cview : UniversalLocalView := localViewU (solMUReplStaticCfg w (j + 1))
  let k : ℝ → ℝ := fun τ => selectorMU_activeSinkP
    bgpHeadlineEta bgpHeadlineEta_pos (bgpHeadlineSolFamNW w) w cview v τ
  let r : ℝ → ℝ := fun τ =>
    selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa τ *
      (Fintype.card UniversalLocalView : ℝ)⁻¹
  let f : ℝ → ℝ := bgpS4bForceNW w j v
  ...
  have hgap : ∀ τ ∈ Icc a b,
      0 ≤ universalPval bgpHeadlineEta bgpHeadlineEta_pos cview
          (((bgpHeadlineSolFamNW w) w).u τ) -
        universalPval bgpHeadlineEta bgpHeadlineEta_pos v
          (((bgpHeadlineSolFamNW w) w).u τ) := by
    intro τ hτ
    have hg := bgpS4b_gapFloor_NW w j v
  ...
  have hsink : ∀ τ ∈ Icc a b, 0 < k τ := by
    ...
    simpa [k, cview, selectorMU_activeSinkP, selectorMU_activeCgP] using
      selectorMU_activeSuffix_sinkCoeff_pos_of_gap_nonnegP
        (p := bgpParamsNW w) (Mcy := bgpHeadlineM)
        (κ₀ := bgpHeadlineKappa) (g₀ := bgpWarmGainQNW w)
        (by norm_num [bgpHeadlineKappa])
        (bgpWarmGainQNW_nonneg_real w) (j + 1) hτsuffix (hgap τ hτ)
  have hk_cont : Continuous k := by
    simpa [k] using selectorMU_activeSink_continuousP
      bgpHeadlineEta bgpHeadlineEta_pos (bgpHeadlineSolFamNW w) w cview v
  have hr_cont : Continuous r := by
    simpa [r] using (selectorMU_activeCr_continuous bgpHeadlineM
      bgpHeadlineKappa).mul continuous_const
  have hlam_cont : Continuous (((bgpHeadlineSolFamNW w) w).lam v) :=
    ((bgpHeadlineSolFamNW w) w).cont_lam v
  have hmean_cont : Continuous fun τ : ℝ =>
      ∑ x : UniversalLocalView,
        ((bgpHeadlineSolFamNW w) w).lam x τ *
          (universalPval bgpHeadlineEta bgpHeadlineEta_pos cview
              (((bgpHeadlineSolFamNW w) w).u τ) -
            universalPval bgpHeadlineEta bgpHeadlineEta_pos x
              (((bgpHeadlineSolFamNW w) w).u τ)) := by
    refine continuous_finsetSum Finset.univ ?_
    intro x _
    exact (((bgpHeadlineSolFamNW w) w).cont_lam x).mul
      (((bgpHeadlineBoxInputsNW w).hP_cont w cview).sub
        ((bgpHeadlineBoxInputsNW w).hP_cont w x))
  have hf_cont : Continuous f := by
    simpa [f, bgpS4bForceNW, cview] using
      ((selectorMU_activeCg_continuousP (p := bgpParamsNW w)
        bgpHeadlineM (bgpWarmGainQNW w)).mul hmean_cont).mul hlam_cont
  ...
  have hm'_cont : ContinuousOn m' (Icc a b) := by
    simpa [m'] using selectorMU_activeQSSDerivRHS_continuousOn_of_sink_posP
      (p := bgpParamsNW w) (sol := bgpHeadlineSolFamNW w) w cview v hsink
  have hy_ode : ∀ τ ∈ Ioo a b,
      HasDerivAt (((bgpHeadlineSolFamNW w) w).lam v)
        (k τ * (r τ / k τ - ((bgpHeadlineSolFamNW w) w).lam v τ) + f τ) τ := by
    intro τ hτ
    have hτIcc : τ ∈ Icc a b := ⟨hτ.1.le, hτ.2.le⟩
    have hk_ne : k τ ≠ 0 := ne_of_gt (hsink τ hτIcc)
    have hlin := bgpNW_lam_hasDerivAt_gapTracking w
      (le_trans ha0 hτIcc.1) cview v
    have hcancel : k τ * (r τ / k τ) = r τ := by
      rw [mul_comm, div_mul_cancel₀ _ hk_ne]
    have heq : k τ * (r τ / k τ - ((bgpHeadlineSolFamNW w) w).lam v τ) +
          f τ = r τ - k τ * ((bgpHeadlineSolFamNW w) w).lam v τ + f τ := by
      rw [mul_sub, hcancel]
    rw [heq]
    simpa [k, r, f, cview, bgpS4bForceNW] using hlin
  have hm_deriv : ∀ τ ∈ Ioo a b,
    ...
    have hq := selectorMU_activeQSS_hasDerivAt_of_sol_u_hasDerivAtP
      (p := bgpParamsNW w) (sol := bgpHeadlineSolFamNW w)
      (by norm_num [bgpHeadlineKappa]) (bgpWarmGainQNW_nonneg_real w)
  ...
  have hstack :=
    stack_write_linear_defect_integral_le_initial_tracking_add_target_deriv_add_forcing_on
      (((bgpHeadlineSolFamNW w) w).lam v) k r f m' a b hab
  ...
  have hcont_rk : Continuous fun τ : ℝ =>
      r τ - k τ * ((bgpHeadlineSolFamNW w) w).lam v τ :=
    hr_cont.sub (hk_cont.mul hlam_cont)
  have hpt : ∀ τ ∈ Icc a b,
      |r τ - k τ * ((bgpHeadlineSolFamNW w) w).lam v τ + f τ| ≤
        |r τ - k τ * ((bgpHeadlineSolFamNW w) w).lam v τ| + |f τ| :=
  ...
  have hint2 : (∫ τ in a..b,
      (|r τ - k τ * ((bgpHeadlineSolFamNW w) w).lam v τ| + |f τ|)) =
      (∫ τ in a..b, |r τ - k τ * ((bgpHeadlineSolFamNW w) w).lam v τ|) +
        ∫ τ in a..b, |f τ| :=
    intervalIntegral.integral_add
      (hcont_rk.abs.intervalIntegrable a b) (hf_cont.abs.intervalIntegrable a b)
  change (∫ τ in a..b,
      |r τ - k τ * ((bgpHeadlineSolFamNW w) w).lam v τ + f τ|) ≤
    |r a / k a - ((bgpHeadlineSolFamNW w) w).lam v a| +
      (∫ τ in a..b, |m' τ|) + 2 * ∫ τ in a..b, |f τ|
  linarith [hint1, hint2, hstack]

private theorem bgpS4b_TV_le_badPair_integralSum_NW
    ...
    (∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
      ...
      (Finset.univ.filter (fun v : UniversalLocalView =>
        ...
        (fun v => ∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
          (selectorMUNextWriteStart j), |bgpS4bDefectNW w j v τ|) := by
  classical
  let a : ℝ := selectorMUEarlyWriteSubStart (j + 1)
  let b : ℝ := selectorMUNextWriteStart j
  let bad : Finset UniversalLocalView := Finset.univ.filter
  ...
  have hab : a ≤ b := by
    ...
    linarith [Real.pi_pos]
  have ha0 : 0 ≤ a := by dsimp [a]; unfold selectorMUEarlyWriteSubStart; positivity
  have hdef_cont : ∀ v, Continuous fun τ => bgpS4bDefectNW w j v τ := by
    intro v
    have hmean : Continuous fun τ : ℝ =>
        ∑ x : UniversalLocalView,
          ((bgpHeadlineSolFamNW w) w).lam x τ *
      ...
      intro x _
      exact (((bgpHeadlineSolFamNW w) w).cont_lam x).mul
        (((bgpHeadlineBoxInputsNW w).hP_cont w _).sub
          ((bgpHeadlineBoxInputsNW w).hP_cont w x))
    have hcr := selectorMU_activeCr_continuous bgpHeadlineM bgpHeadlineKappa
    have hsink := selectorMU_activeSink_continuousP bgpHeadlineEta
      bgpHeadlineEta_pos (bgpHeadlineSolFamNW w) w
      (localViewU (solMUReplStaticCfg w (j + 1))) v
    have hcg := selectorMU_activeCg_continuousP (p := bgpParamsNW w)
      bgpHeadlineM (bgpWarmGainQNW w)
    simpa [bgpS4bDefectNW, bgpS4bForceNW] using
      ((hcr.mul continuous_const).sub
        (hsink.mul (((bgpHeadlineSolFamNW w) w).cont_lam v))).add
      ((hcg.mul hmean).mul (((bgpHeadlineSolFamNW w) w).cont_lam v))
  have hsum_cont : Continuous fun τ => bad.sum
      (fun v => |bgpS4bDefectNW w j v τ|) :=
  ...
  have hmono := intervalIntegral.integral_mono_on hab
    ((selectorMU_mixTargetDerivRHS_continuous_for_edgeP
      (sol := bgpHeadlineSolFamNW w) w haltCoordU).abs.intervalIntegrable a b)
    (hsum_cont.intervalIntegrable (μ := MeasureTheory.volume) a b)
    (fun τ hτ => by
      have hp := bgpS4b_absRHS_le_badPair_sum_NW w j henc
        (le_trans ha0 hτ.1)
      simpa [bad, bgpS4bDefectNW, bgpS4bForceNW] using hp)
  have hswap := intervalIntegral.integral_finsetSum
    (s := bad) (f := fun v τ => |bgpS4bDefectNW w j v τ|)
  ...
  calc
    (∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
      ...
      |SelectorReplicatorDynSol.mixTargetDerivRHS
        ((bgpHeadlineSolFamNW w) w) τ haltCoordU|)
      ≤ ∫ τ in a..b, bad.sum (fun v => |bgpS4bDefectNW w j v τ|) := by
        simpa [a, b] using hmono
    _ = bad.sum (fun v => ∫ τ in a..b, |bgpS4bDefectNW w j v τ|) := hswap
    _ = _ := by simp [bad, a, b]

/-! ## Right suffix anchor currencies -/

private theorem bgpHoff_early_exp_currency_le_budget_div_32_NW
    (w j : ℕ) {K : ℝ}
    (hK : K ≤ Real.exp (100 * (bgpScaleW w : ℝ) + 1000)) :
...
private theorem bgpS4b_badMass_early_le_budget_div_32_NW (w j : ℕ) :
    bgpHoffBadPairNW w j (selectorMUEarlyWriteSubStart (j + 1)) ≤
      selectorMUHoffEdgeBudget3992 j / 32 := by
  let a : ℝ := selectorMUEarlyWriteSubStart (j + 1)
  have ha0 : 0 ≤ a := by dsimp [a]; unfold selectorMUEarlyWriteSubStart; positivity
  have haFull : a ∈ Icc (selectorMUZOffEnd j) (selectorMUNextWriteStart j) := by
    ...
    · dsimp [a]
      ...
      push_cast
      linarith [Real.pi_pos]
  have hmass := (bgpHoffBadPair_le_newLoser_NW w j ha0).trans
    (bgpS4b_loser_mass_timelocal_NW w j a haFull)
  have hcurrency := bgpHoff_early_exp_currency_le_budget_div_32_NW w j
    (bgpS4bLoserCoeffNW_le_exp_currency w j)
  exact hmass.trans (by simpa [a] using hcurrency)

private theorem bgpS4b_qssAnchor_early_le_budget_div_32_NW (w j : ℕ) :
    (Finset.univ.filter (fun v : UniversalLocalView =>
      v ≠ localViewU (solMUReplStaticCfg w j) ∧
        v ≠ localViewU (solMUReplStaticCfg w (j + 1)))).sum
      (fun v =>
  ...
  let g : ℝ := ((bgpWarmGainQNW w : ℚ) : ℝ)
  let card : ℝ := (Fintype.card UniversalLocalView : ℝ)
  let floor : ℝ := (1 / 2 : ℝ) ^ bgpHeadlineM *
  ...
  have hsin : 0 ≤ Real.sin a := by
    apply bgpS4b_sin_nonneg j haFull.1 haFull.2
  have hview : ∀ v ∈ (Finset.univ.filter (fun v : UniversalLocalView =>
      v ≠ localViewU (solMUReplStaticCfg w j) ∧
        v ≠ localViewU (solMUReplStaticCfg w (j + 1)))),
      |selectorMU_activeQSSP bgpHeadlineEta bgpHeadlineEta_pos
        (bgpHeadlineSolFamNW w) w cview v a| ≤ B := by
    intro v hv
    have hvNew := (Finset.mem_filter.mp hv).2.2
    have hgap := bgpS4b_gapFloor_NW w j v hvNew a haFull
    have hsink_ge := bgpS4_sink_ge_of_gap_half_NW w cview v hsin hgap
    have hsink_pos : 0 < selectorMU_activeSinkP
        bgpHeadlineEta bgpHeadlineEta_pos (bgpHeadlineSolFamNW w) w
          cview v a := lt_of_lt_of_le (by simpa [floor, g] using hfloor) hsink_ge
    have hcr0 : 0 ≤ selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa a := by
      unfold selectorMU_activeCr
      exact mul_nonneg (pow_nonneg (by nlinarith [Real.neg_one_le_cos a]) _)
        (by norm_num [bgpHeadlineKappa])
    have hcr1 : selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa a ≤ 1 := by
      unfold selectorMU_activeCr
      rw [show ((bgpHeadlineKappa : ℚ) : ℝ) = 1 by
        norm_num [bgpHeadlineKappa], mul_one]
    ...
    calc
      selectorMU_activeCr bgpHeadlineM bgpHeadlineKappa a * card⁻¹
          ≤ 1 * card⁻¹ := mul_le_mul_of_nonneg_right hcr1 (inv_nonneg.mpr hcard.le)
      _ = card⁻¹ := one_mul _
      _ = B * floor := hBfloor.symm
      _ ≤ B * selectorMU_activeSinkP bgpHeadlineEta bgpHeadlineEta_pos
          (bgpHeadlineSolFamNW w) w cview v a := by
        apply mul_le_mul_of_nonneg_left _ hB0
        simpa [floor, g] using hsink_ge
  have hsum : (Finset.univ.filter (fun v : UniversalLocalView =>
      v ≠ localViewU (solMUReplStaticCfg w j) ∧
        v ≠ localViewU (solMUReplStaticCfg w (j + 1)))).sum
      (fun v => |selectorMU_activeQSSP bgpHeadlineEta bgpHeadlineEta_pos
        (bgpHeadlineSolFamNW w) w cview v a|) ≤ card * B := by
    calc
      _ ≤ (Finset.univ.filter (fun v : UniversalLocalView =>
          v ≠ localViewU (solMUReplStaticCfg w j) ∧
  ...
  have hcardB : card * B =
    ...
    dsimp [B, floor]
    have hpow : ((1 : ℝ) / 2) ^ bgpHeadlineM = 1 / 1048576 := by
      norm_num [bgpHeadlineM]
  ...
  have hK : 2097152 / g ≤ Real.exp (100 * (bgpScaleW w : ℝ) + 1000) := by
    ...
    exact hdiv.trans (hpowexp.trans (Real.exp_le_exp.mpr (by
      nlinarith [bgpNW4_S_ge w])))
  have hcurrency := bgpHoff_early_exp_currency_le_budget_div_32_NW w j hK
  calc
...
private theorem bgpS4b_qssDerivMass_suffix_le_NW (w j : ℕ) :
    (Finset.univ.filter (fun v : UniversalLocalView =>
      v ≠ localViewU (solMUReplStaticCfg w j) ∧
        v ≠ localViewU (solMUReplStaticCfg w (j + 1)))).sum
      (fun v => ∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
  ...
  have hsub : bad.sum (fun v => ∫ τ in E..H,
    ...
    have hsink : ∀ τ ∈ Icc Z H,
        0 < selectorMU_activeSinkP bgpHeadlineEta bgpHeadlineEta_pos
          (bgpHeadlineSolFamNW w) w
          (localViewU (solMUReplStaticCfg w (j + 1))) v τ := by
      intro τ hτ
      have hτ' : τ ∈ Icc (selectorMUZOffEnd j) (selectorMUNextWriteStart j) :=
        by simpa [Z, H] using hτ
      have hsin := bgpS4b_sin_nonneg j hτ'.1 hτ'.2
      have hgap := bgpS4b_gapFloor_NW w j v hvNew τ hτ'
      have hf := bgpS4_sink_ge_of_gap_half_NW w _ v hsin hgap
      have hpos : (0 : ℝ) < (1 / 2 : ℝ) ^ bgpHeadlineM *
          (((bgpWarmGainQNW w : ℚ) : ℝ) *
  ...
  exact hsub'.trans (bgpS4b_qssDerivMass_le_NW w j)

private theorem bgpS4b_suffixTV_le_budget_div_8_NW
    (w j : ℕ) (henc : selectorMUHaltEncConst (solMUReplStaticCfg w) j) :
    (∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
        (selectorMUNextWriteStart j),
      |SelectorReplicatorDynSol.mixTargetDerivRHS
  ...
  let bad : Finset UniversalLocalView := Finset.univ.filter
    (fun v : UniversalLocalView =>
      v ≠ localViewU (solMUReplStaticCfg w j) ∧
        v ≠ localViewU (solMUReplStaticCfg w (j + 1)))
  let a : ℝ := selectorMUEarlyWriteSubStart (j + 1)
  have hred := bgpS4b_TV_le_badPair_integralSum_NW w j henc
  have hsum := Finset.sum_le_sum
    (fun v (hv : v ∈ bad) =>
      bgpS4b_perView_TV_le_NW w j v (Finset.mem_filter.mp hv).2.2)
  rw [Finset.sum_add_distrib, Finset.sum_add_distrib, ← Finset.mul_sum] at hsum
  have hanchor := Finset.sum_le_sum
    (fun v (_hv : v ∈ bad) => by
      have hlam : 0 ≤ ((bgpHeadlineSolFamNW w) w).lam v a := by
        apply bgpNW_lam_nonneg_forward w w v a
        dsimp [a]
        unfold selectorMUEarlyWriteSubStart
        positivity
      calc
        |selectorMU_activeQSSP bgpHeadlineEta bgpHeadlineEta_pos
          ...
          ((bgpHeadlineSolFamNW w) w).lam v a|
          ≤ |selectorMU_activeQSSP bgpHeadlineEta bgpHeadlineEta_pos
              (bgpHeadlineSolFamNW w) w
              (localViewU (solMUReplStaticCfg w (j + 1))) v a| +
            |(-((bgpHeadlineSolFamNW w) w).lam v a)| := by
              simpa [sub_eq_add_neg] using abs_add_le
                (selectorMU_activeQSSP bgpHeadlineEta bgpHeadlineEta_pos
                  (bgpHeadlineSolFamNW w) w
                  (localViewU (solMUReplStaticCfg w (j + 1))) v a)
                (-((bgpHeadlineSolFamNW w) w).lam v a)
        _ = |selectorMU_activeQSSP bgpHeadlineEta bgpHeadlineEta_pos
              (bgpHeadlineSolFamNW w) w
              (localViewU (solMUReplStaticCfg w (j + 1))) v a| +
            ((bgpHeadlineSolFamNW w) w).lam v a := by
              rw [abs_neg, abs_of_nonneg hlam])
  rw [Finset.sum_add_distrib] at hanchor
  have hqssA := bgpS4b_qssAnchor_early_le_budget_div_32_NW w j
  have hlamA := bgpS4b_badMass_early_le_budget_div_32_NW w j
  have hqssD := bgpS4b_qssDerivMass_suffix_le_NW w j
  have hforce := bgpS4b_forcingMass_le_NW w j
  have hforce' : 2 * (Finset.univ.filter (fun v : UniversalLocalView =>
      v ≠ localViewU (solMUReplStaticCfg w j) ∧
        v ≠ localViewU (solMUReplStaticCfg w (j + 1)))).sum
      (fun v => ∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
        (selectorMUNextWriteStart j), |bgpS4bForceNW w j v τ|) ≤
      selectorMUHoffEdgeBudget3992 j / 32 := by
    simpa only [bgpS4bForceNW] using hforce
  have hmassEq : bad.sum (fun v =>
      ((bgpHeadlineSolFamNW w) w).lam v a) = bgpHoffBadPairNW w j a := by
    rfl
  rw [hmassEq] at hanchor
  have hb0 := bgpHeadlineHoff_edgeBudget3992_nonneg j
  dsimp only [bad, a] at hsum hanchor
  linarith [hred, hsum, hanchor, hqssA, hlamA, hqssD, hforce', hb0]

/-! ## Settled read endpoint -/

private theorem bgpHoff_zGateMass_ge_NW (w j : ℕ) :
    Real.pi / 24 * Real.exp (50 * (bgpScaleW w : ℝ) * bgpS13Mid j) ≤
      ∫ τ in (bgpS13Mid j)..(selectorMUWriteReadTime j),
        (bgpParamsNW w).A * ((bgpHeadlineSolFamNW w) w).α τ *
          bGateZ (bgpParamsNW w).L (((bgpHeadlineSolFamNW w) w).μ τ) τ := by
  have hab := bgpS13_mid_le_read j
  have ha0 := bgpS13_mid_nonneg j
  have hpoint : ∀ τ ∈ Icc (bgpS13Mid j) (selectorMUWriteReadTime j),
      Real.exp (50 * (bgpScaleW w : ℝ) * bgpS13Mid j) ≤
        (bgpParamsNW w).A * ((bgpHeadlineSolFamNW w) w).α τ *
          bGateZ (bgpParamsNW w).L (((bgpHeadlineSolFamNW w) w).μ τ) τ := by
    intro τ hτ
    have hτ0 := le_trans ha0 hτ.1
    have hsin : (1 / 2 : ℝ) ≤ Real.sin τ :=
      bgpS13_sin_ge_half j
        (le_trans (le_trans (bgpS13_selectStart_le_subEnd j)
          (bgpS13_subEnd_le_mid j)) hτ.1) hτ.2
    exact (Real.exp_le_exp.mpr
      (mul_le_mul_of_nonneg_left hτ.1
        (mul_nonneg (by norm_num) (bgpScaleWR_pos w).le))).trans
      (bgpNW4_zKernel_ge_of_sin_ge_half w w hτ0 hsin)
  have hmono := intervalIntegral.integral_mono_on hab
    (continuous_const.intervalIntegrable (μ := MeasureTheory.volume) _ _)
    ((bgpNW4_zKernel_continuous w w).intervalIntegrable
      (μ := MeasureTheory.volume) _ _) hpoint
  have hlen : selectorMUWriteReadTime j - bgpS13Mid j = Real.pi / 24 := by
    unfold selectorMUWriteReadTime bgpS13Mid
    ring
  rw [intervalIntegral.integral_const, smul_eq_mul, hlen] at hmono
  exact hmono

private theorem bgpHoff_z_readStart_le_budget_div_32_NW
    (w j : ℕ) :
    |((bgpHeadlineSolFamNW w) w).z (selectorMUInterReadStart j) haltCoordU -
        stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤
      selectorMUHoffEdgeBudget3992 j / 32 := by
  classical
  haveI : Nonempty UniversalLocalView := ⟨defaultLocalViewU⟩
  ...
  let K : ℝ := bgpS4aLoserCoeffNW w j
  have hab : a ≤ b := by simpa [a, b] using bgpS13_mid_le_read j
  have ha0 : 0 ≤ a := by simpa [a] using bgpS13_mid_nonneg j
  have hS : (5184 : ℝ) ≤ S := by dsimp [S]; exact bgpNW4_S_ge w
  have hS0 : 0 < S := by dsimp [S]; exact bgpScaleWR_pos w
  have hK := bgpS4aLoserCoeffNW_le_exp_currency w j
  have hmargin : 100 * S + 1000 + 200 * T ≤ 300 * S * a := by
    dsimp only [T, a]
    unfold bgpS13Mid
    have hj : (0 : ℝ) ≤ (j : ℝ) := Nat.cast_nonneg j
    ...
    have hexp : K * Real.exp (-((bgpParamsNW w).cα * t)) ≤ rate := by
      ...
      calc
        ...
        _ ≤ Real.exp (-(200 * T)) := by
          ...
          linarith
        _ = rate := rfl
    have hmass' : bgpHoffOldLoserNW w j t ≤
        K * Real.exp (-((bgpParamsNW w).cα * t)) := by
      simpa only [bgpHoffOldLoserNW, K] using hmass
  ...
  have hmix : ∀ t ∈ Icc a b,
      |selectorMixTarget branchU ((bgpHeadlineSolFamNW w) w).u
        ...
        rate := by
    intro t ht
    exact (bgpHoff_mix_pointwise_le_oldLoser_NW w j
  ...
  have hk0 : ∀ t ∈ Icc a b,
      0 ≤ (bgpParamsNW w).A * ((bgpHeadlineSolFamNW w) w).α t *
        bGateZ (bgpParamsNW w).L (((bgpHeadlineSolFamNW w) w).μ t) t :=
    fun t ht => bgpNW4_zKernel_nonneg w w (le_trans ha0 ht.1)
  have hzero : ∀ t ∈ Icc b b,
      |((bgpHeadlineSolFamNW w) w).z t haltCoordU -
        ((bgpHeadlineSolFamNW w) w).z b haltCoordU| ≤ (0 : ℝ) := by
    intro t ht
    have ht' : t = b := le_antisymm ht.2 ht.1
    subst t
    simp
  have hzraw := z_after_write_bound_repl
    (sol := (bgpHeadlineSolFamNW w) w) (s := haltCoordU)
    (a := a) (m := b) (b := b)
    (M := stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU)
    (δw := rate) (δzh := 0) hab hdom
    (bgpNW4_zKernel_continuous w w) hk0 hmix hzero b ⟨le_rfl, le_rfl⟩
  let I : ℝ := ∫ t in a..b,
      (bgpParamsNW w).A * ((bgpHeadlineSolFamNW w) w).α t *
        bGateZ (bgpParamsNW w).L (((bgpHeadlineSolFamNW w) w).μ t) t
  have hmass := bgpHoff_zGateMass_ge_NW w j
  ...
  have hexpI : Real.exp (-I) ≤ rate := by
    ...
    dsimp [rate]
    linarith
  have hMunit := bgp_enc_haltCoordU_mem_unit (solMUReplStaticCfg w (j + 1))
  have hzunit := (bgpHeadlineBoxInputsNW w).halt_z_mem_Icc
    (by rw [bgpParamsNW_A_eq]; norm_num) w a ha0
  have hinit : |((bgpHeadlineSolFamNW w) w).z a haltCoordU -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤ 1 :=
    bgp_abs_sub_le_one_of_unit_interval_pair hzunit hMunit
  ...
  have hz : |((bgpHeadlineSolFamNW w) w).z b haltCoordU -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤
      2 * rate := by
    have hzraw' := hzraw
  ...
  rw [show bgpParams38.cμ * (1 / 2 : ℝ) ^ bgpParams38.L -
      bgpParams38.cα = 200 by norm_num [bgpParams38]]
  change |((bgpHeadlineSolFamNW w) w).z b haltCoordU -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤
    (selectorMUHoffEdgeBudgetCoeff * Real.exp (-(200 * T))) / 32
...
private theorem bgpHoff_z_drift_read_to_zOffEnd_NW (w j : ℕ) :
    |((bgpHeadlineSolFamNW w) w).z (selectorMUZOffEnd j) haltCoordU -
        ((bgpHeadlineSolFamNW w) w).z (selectorMUInterReadStart j) haltCoordU| ≤
      bgpHeadlineHoffCapLeftFieldNW w j +
  ...
  have hInt : ∀ x x' : ℝ, IntervalIntegrable
      (fun τ => bgpHeadlineHoffIntegrandNW w τ) MeasureTheory.volume x x' :=
    fun x x' => (selectorMUHoffIntegrandP_continuous
      (p := bgpParamsNW w) (sol := sol) w).intervalIntegrable x x'
  have habs_eq : ∀ τ,
      |k τ * (m τ - y τ)| = bgpHeadlineHoffIntegrandNW w τ := by
    intro τ
    simp [bgpHeadlineHoffIntegrandNW, selectorMUHoffIntegrandP,
      selectorMUHoffGateCoeffP, k, m, y, sol, mul_assoc]
  have habs : |y c - y a| ≤ ∫ τ in a..c, bgpHeadlineHoffIntegrandNW w τ := by
    rw [← hftc]
    refine (intervalIntegral.abs_integral_le_integral_abs hac).trans ?_
    exact le_of_eq (intervalIntegral.integral_congr
      (fun τ _ => habs_eq τ))
  have hsplit : (∫ τ in a..c, bgpHeadlineHoffIntegrandNW w τ) =
      (∫ τ in a..b, bgpHeadlineHoffIntegrandNW w τ) +
        ∫ τ in b..c, bgpHeadlineHoffIntegrandNW w τ :=
    (intervalIntegral.integral_add_adjacent_intervals (hInt a b) (hInt b c)).symm
  have hmid := bgpHeadlineHoff_middle_fieldIntegral_NW
    bgpHeadlineBoxInputsNW w j c ⟨by simpa [b, c] using hbc, le_rfl⟩
  calc
    |((bgpHeadlineSolFamNW w) w).z (selectorMUZOffEnd j) haltCoordU -
        ((bgpHeadlineSolFamNW w) w).z (selectorMUInterReadStart j) haltCoordU|
      = |y c - y a| := rfl
    _ ≤ ∫ τ in a..c, bgpHeadlineHoffIntegrandNW w τ := habs
    _ = (∫ τ in a..b, bgpHeadlineHoffIntegrandNW w τ) +
        ∫ τ in b..c, bgpHeadlineHoffIntegrandNW w τ := hsplit
    _ ≤ bgpHeadlineHoffCapLeftFieldNW w j +
        selectorMUHoffMiddleEnvelopeFullCapNW w j := by
      exact add_le_add le_rfl (by simpa [b, c] using hmid)

private theorem bgpHoff_zoff_exp_currency_le_budget_div_32_NW
    (w j : ℕ) {K : ℝ}
    (hK : K ≤ Real.exp (100 * (bgpScaleW w : ℝ) + 1000)) :
...
private theorem bgpS4b_badMass_right_le_budget_div_32_NW
    (w j : ℕ) {t : ℝ}
    (ht : t ∈ Icc (selectorMUZOffEnd j) (selectorMUNextWriteStart j)) :
    bgpHoffBadPairNW w j t ≤ selectorMUHoffEdgeBudget3992 j / 32 := by
  have ht0 : 0 ≤ t := le_trans (by unfold selectorMUZOffEnd; positivity) ht.1
  have hmass := (bgpHoffBadPair_le_newLoser_NW w j ht0).trans
    (bgpS4b_loser_mass_timelocal_NW w j t ht)
  have hmono : bgpS4bLoserCoeffNW w j *
      Real.exp (-((bgpParamsNW w).cα * t)) ≤
      bgpS4bLoserCoeffNW w j *
        Real.exp (-((bgpParamsNW w).cα * selectorMUZOffEnd j)) := by
    apply mul_le_mul_of_nonneg_left _ (bgpS4bLoserCoeffNW_nonneg w j)
    apply Real.exp_le_exp.mpr
    exact neg_le_neg (mul_le_mul_of_nonneg_left ht.1 (bgpParamsNW_cα_pos w).le)
  exact hmass.trans (hmono.trans
    (bgpHoff_zoff_exp_currency_le_budget_div_32_NW w j
      (bgpS4bLoserCoeffNW_le_exp_currency w j)))

private theorem bgpHoff_z_early_le_zoff_add_budget_div_32_NW
    (w j : ℕ) (henc : selectorMUHaltEncConst (solMUReplStaticCfg w) j) :
    |((bgpHeadlineSolFamNW w) w).z (selectorMUEarlyWriteSubStart (j + 1))
          haltCoordU -
        stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤
      |((bgpHeadlineSolFamNW w) w).z (selectorMUZOffEnd j) haltCoordU -
        stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| +
  ...
  have hkcont : Continuous k := by simpa [k, sol] using bgpNW4_zKernel_continuous w w
  have hk0 : ∀ t ∈ Icc Z E, 0 ≤ k t := by
    intro t ht
    simpa [k, sol] using bgpNW4_zKernel_nonneg w w (le_trans hZ0 ht.1)
  ...
  have hmsup : ∀ t ∈ Icc Z E, |m t - M| ≤ δ := by
    ...
    have htfull : t ∈ Icc (selectorMUZOffEnd j) (selectorMUNextWriteStart j) :=
      ⟨by simpa [Z] using ht.1, le_trans (by simpa [E] using ht.2) (by
        ...
        push_cast
        linarith [Real.pi_pos])⟩
    exact (bgpHoff_mix_pointwise_le_badPair_NW w j
      (le_trans hZ0 ht.1) henc).trans
      (bgpS4b_badMass_right_le_budget_div_32_NW w j htfull)
  ...
  have hδ0 : 0 ≤ δ := by
    dsimp [δ]
    exact div_nonneg (bgpHeadlineHoff_edgeBudget3992_nonneg j) (by norm_num)
...
private theorem bgpHoff_suffixField_le_initial_add_TV_NW (w j : ℕ) :
    (∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
        (selectorMUNextWriteStart j), bgpHeadlineHoffIntegrandNW w τ) ≤
      |selectorMixTarget branchU ((bgpHeadlineSolFamNW w) w).u
      ...
      (∫ τ in (selectorMUEarlyWriteSubStart (j + 1))..
        (selectorMUNextWriteStart j),
        |SelectorReplicatorDynSol.mixTargetDerivRHS
          ((bgpHeadlineSolFamNW w) w) τ haltCoordU|) := by
  let a : ℝ := selectorMUEarlyWriteSubStart (j + 1)
  let b : ℝ := selectorMUNextWriteStart j
  let sol := bgpHeadlineSolFamNW w
  let y : ℝ → ℝ := fun τ => (sol w).z τ haltCoordU
  ...
  have hk0 : ∀ τ ∈ Icc a b, 0 ≤ k τ := by
    ...
    simpa [k, sol, selectorMUHoffGateCoeffP] using
      bgpNW4_zKernel_nonneg w w (le_trans ha0 hτ.1)
  ...
  have hcap : (∫ τ in a..b, bgpHeadlineHoffIntegrandNW w τ) ≤
      ∫ τ in a..b, k τ * |m τ - y τ| := by
    apply intervalIntegral.integral_mono_on hab
...
private theorem bgpHoff_middleCapNW_le_budget_div_32 (w j : ℕ) :
  ...
  (selectorMUHoffMiddleEnvelopeFullCapNW_le_old_middle_budget w j).trans
    (bgpHeadlineHoff_middleFullCap_le_edgeBudget_div_32 j)

private theorem bgpHoff_capLeft_le_three_budget_div_32 (w j : ℕ) :
    bgpHeadlineHoffCapLeftFieldNW w j ≤
  ...
  let e : ℝ → ℝ := fun τ =>
    |selectorMixTarget branchU ((bgpHeadlineSolFamNW w) w).u
        ((bgpHeadlineSolFamNW w) w).lam τ haltCoordU -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU|
  let loser : ℝ → ℝ := bgpHoffOldLoserNW w j
  have hkcont : Continuous k := by
    simpa [k] using selectorMUHoffGateCoeffP_continuous
      (p := bgpParamsNW w) (sol := bgpHeadlineSolFamNW w) w
  have hecont : Continuous e :=
    (((bgpHeadlineSolFamNW w) w).cont_mixTarget haltCoordU).sub
      continuous_const |>.abs
  have hlcont : Continuous loser := by
    dsimp [loser, bgpHoffOldLoserNW]
    exact continuous_finsetSum _ (fun v _ =>
      ((bgpHeadlineSolFamNW w) w).cont_lam v)
  ...
  have hmono : (∫ τ in (selectorMUInterReadStart j)..(selectorMUZOffStart j),
    ...
    intro τ hτ
    exact mul_le_mul_of_nonneg_left
      (bgpHoff_mix_pointwise_le_oldLoser_NW w j (le_trans ha0 hτ.1))
      (hk0 τ hτ)
  have hbase := selectorMUHoff_fieldIntegral_le_initial_add_targetP
    (p := bgpParamsNW w) (sol := bgpHeadlineSolFamNW w)
    (by rw [bgpParamsNW_A_eq]; norm_num) w hab ha0
    (M := stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU)
  have hz := bgpHoff_z_readStart_le_budget_div_32_NW w j
  have hbad := bgpHoff_left_oldLoserIntegral_le_budget_div_32_NW w j
  have hbase' : bgpHeadlineHoffCapLeftFieldNW w j ≤
      |((bgpHeadlineSolFamNW w) w).z (selectorMUInterReadStart j) haltCoordU -
        stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| +
      2 * (∫ τ in (selectorMUInterReadStart j)..(selectorMUZOffStart j),
        k τ * e τ) := by
    simpa [bgpHeadlineHoffCapLeftFieldNW,
      bgpHeadlineHoffIntegrandNW, k, e] using hbase
  have hmono' : (∫ τ in (selectorMUInterReadStart j)..(selectorMUZOffStart j),
      k τ * e τ) ≤ selectorMUHoffEdgeBudget3992 j / 32 := by
    exact hmono.trans (by simpa [k, loser] using hbad)
  linarith

private theorem bgpHoff_zoff_error_le_five_budget_div_32_NW
    (w j : ℕ) (henc : selectorMUHaltEncConst (solMUReplStaticCfg w) j) :
    |((bgpHeadlineSolFamNW w) w).z (selectorMUZOffEnd j) haltCoordU -
        stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤
      5 * (selectorMUHoffEdgeBudget3992 j / 32) := by
  have hread := bgpHoff_z_readStart_le_budget_div_32_NW w j
  have hdrift := bgpHoff_z_drift_read_to_zOffEnd_NW w j
  have hleft := bgpHoff_capLeft_le_three_budget_div_32 w j
  have hmid := bgpHoff_middleCapNW_le_budget_div_32 w j
...
private theorem bgpHoff_early_mix_z_le_seven_budget_div_32_NW
    (w j : ℕ) (henc : selectorMUHaltEncConst (solMUReplStaticCfg w) j) :
    |selectorMixTarget branchU ((bgpHeadlineSolFamNW w) w).u
        ((bgpHeadlineSolFamNW w) w).lam
        (selectorMUEarlyWriteSubStart (j + 1)) haltCoordU -
  ...
  let M : ℝ := stackMachineEncodingU.enc
    (solMUReplStaticCfg w (j + 1)) haltCoordU
  have hZ := bgpHoff_zoff_error_le_five_budget_div_32_NW w j henc
  have hzE := bgpHoff_z_early_le_zoff_add_budget_div_32_NW w j henc
  have hE0 : 0 ≤ E := by dsimp [E]; unfold selectorMUEarlyWriteSubStart; positivity
  have hmix := (bgpHoff_mix_pointwise_le_badPair_NW w j hE0 henc).trans
    (bgpS4b_badMass_early_le_budget_div_32_NW w j)
  have htri := abs_sub_le
    (selectorMixTarget branchU ((bgpHeadlineSolFamNW w) w).u
      ((bgpHeadlineSolFamNW w) w).lam E haltCoordU) M
    (((bgpHeadlineSolFamNW w) w).z E haltCoordU)
  have hcomm := abs_sub_comm M
    (((bgpHeadlineSolFamNW w) w).z E haltCoordU)
  dsimp only [E, M] at hmix htri hcomm ⊢
  linarith

private theorem bgpHoff_capRight_le_eighteen_budget_div_32_NW
    (w j : ℕ) (henc : selectorMUHaltEncConst (solMUReplStaticCfg w) j) :
    bgpHeadlineHoffCapRightFieldNW w j ≤
  ...
  have hEH : E ≤ H := by
    ...
    linarith [Real.pi_pos]
  have hZ0 : 0 ≤ Z := by dsimp [Z]; unfold selectorMUZOffEnd; positivity
  have hcross := bgpHoff_fieldCap_le_readError_add_badIntegral_NW
    w j hZE hZ0 henc
  have hzZ := bgpHoff_zoff_error_le_five_budget_div_32_NW w j henc
  have hbad := bgpHoff_right_badIntegral_le_budget_div_32_NW w j
  have hcrossCap : (∫ τ in Z..E, bgpHeadlineHoffIntegrandNW w τ) ≤
      7 * (selectorMUHoffEdgeBudget3992 j / 32) := by
    dsimp only [Z, E] at hcross hzZ hbad ⊢
    linarith
  have hsuffix := bgpHoff_suffixField_le_initial_add_TV_NW w j
  have hmixz := bgpHoff_early_mix_z_le_seven_budget_div_32_NW w j henc
  have htv := bgpS4b_suffixTV_le_budget_div_8_NW w j henc
  have hsuffixCap : (∫ τ in E..H, bgpHeadlineHoffIntegrandNW w τ) ≤
  ...
  change (∫ τ in Z..H, bgpHeadlineHoffIntegrandNW w τ) ≤ _
  have heq : (∫ τ in Z..H, bgpHeadlineHoffIntegrandNW w τ) =
      (∫ τ in Z..E, bgpHeadlineHoffIntegrandNW w τ) +
        ∫ τ in E..H, bgpHeadlineHoffIntegrandNW w τ := hsplit.symm
  rw [heq]
  linarith

private theorem bgpHoff_capLeft_le_half_NW
    (w j : ℕ) :
    bgpHeadlineHoffCapLeftFieldNW w j ≤
      selectorMUHoffEdgeBudget3992 j / 2 := by
  have h := bgpHoff_capLeft_le_three_budget_div_32 w j
  have hb0 := bgpHeadlineHoff_edgeBudget3992_nonneg j
  linarith

private theorem bgpHoff_capRight_le_nineteen_budget_div_32_NW
    (w j : ℕ) (henc : selectorMUHaltEncConst (solMUReplStaticCfg w) j) :
    bgpHeadlineHoffCapRightFieldNW w j ≤
      19 * (selectorMUHoffEdgeBudget3992 j / 32) := by
  exact (bgpHoff_capRight_le_eighteen_budget_div_32_NW w j henc).trans
    (by have hb0 := bgpHeadlineHoff_edgeBudget3992_nonneg j; linarith)

def bgpHeadlineHoffResidualNW : Paper3HeadlineHoffFieldIntegralResidualNW :=
  bgpHeadlineHoffFieldIntegralResidualNW_of_caps
    bgpHeadlineBoxInputsNW
    (fun w j => 3 * (selectorMUHoffEdgeBudget3992 j / 32))
    (fun w j => 19 * (selectorMUHoffEdgeBudget3992 j / 32))
    bgpHoff_capLeft_le_three_budget_div_32
    bgpHoff_capRight_le_nineteen_budget_div_32_NW
    (fun w j henc => by
      have hb0 := bgpHeadlineHoff_edgeBudget3992_nonneg j
      linarith)
