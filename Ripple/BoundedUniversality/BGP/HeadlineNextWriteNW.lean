...
noncomputable def bgpNextWriteEarlyDeltaGFloorNW (w j : ℕ) : ℝ :=
  (3 / 4 : ℝ) ^ bgpHeadlineM *
    (((bgpWarmGainQNW w : ℚ) : ℝ) *
      Real.exp ((bgpParamsNW w).cα * selectorMUSelectStartTime j)) *
    (Real.pi / 12 - selectorMURecoveryDelta)

theorem bgpNextWriteEarlyDeltaGFloorNW_nonneg (w j : ℕ) :
    0 ≤ bgpNextWriteEarlyDeltaGFloorNW w j := by
  unfold bgpNextWriteEarlyDeltaGFloorNW
...
floor. -/
theorem bgpNextWriteEarlyDeltaGFloorNW_le (w j : ℕ) {t : ℝ}
    (ht : selectorMUEarlyWriteSubStart j ≤ t) :
    bgpNextWriteEarlyDeltaGFloorNW w j ≤
      ((bgpHeadlineSolFamNW w) w).G t -
        ((bgpHeadlineSolFamNW w) w).G (selectorMUSelectStartTime j) := by
  ...
  have h1 := bgpNW4_G_sub_eq w w hsel0 hsel_early
  have h2 := bgpNW4_G_sub_eq w w hearly0 ht
  have h2_nonneg : 0 ≤ ((bgpHeadlineSolFamNW w) w).G t -
      ((bgpHeadlineSolFamNW w) w).G
        (selectorMUEarlyWriteSubStart j) := by
    rw [h2]
    exact intervalIntegral.integral_nonneg ht
      (fun s _ => bgpNW4cg_nonneg w s)
  have hfloor : bgpNextWriteEarlyDeltaGFloorNW w j ≤
      ((bgpHeadlineSolFamNW w) w).G (selectorMUEarlyWriteSubStart j) -
        ((bgpHeadlineSolFamNW w) w).G
        ...
        (3 / 4 : ℝ) ^ bgpHeadlineM *
          (((bgpWarmGainQNW w : ℚ) : ℝ) *
            Real.exp ((bgpParamsNW w).cα *
              selectorMUSelectStartTime j)) ≤ bgpNW4cg w s := by
      intro s hs
...
noncomputable def bgpNextWriteEarlyEpsNW (w j : ℕ) : ℝ :=
  ((Fintype.card UniversalLocalView : ℝ) - 1) *
    ((Fintype.card UniversalLocalView : ℝ) *
      Real.exp (-(15 / 16) * bgpNextWriteEarlyDeltaGFloorNW w j) +
     2 ^ (bgpHeadlineM + 1) *
      Real.exp (-((bgpParamsNW w).cα * selectorMUSelectStartTime j)))

theorem bgpNextWriteEarlyEpsNW_nonneg (w j : ℕ) :
    0 ≤ bgpNextWriteEarlyEpsNW w j := by
  classical
  unfold bgpNextWriteEarlyEpsNW
...
theorem bgpNextWriteEarlyEpsNW_tendsto_zero (w : ℕ) :
    Tendsto (bgpNextWriteEarlyEpsNW w) atTop (𝓝 0) := by
  classical
  have hfloor := bgpNextWriteEarlyDeltaGFloorNW_tendsto_atTop w
  have hfloor_scaled : Tendsto
      (fun j => (15 / 16 : ℝ) * bgpNextWriteEarlyDeltaGFloorNW w j)
      atTop atTop := Filter.Tendsto.const_mul_atTop (by norm_num) hfloor
  have hdecay1 : Tendsto
      (fun j => Real.exp (-(15 / 16 : ℝ) *
        bgpNextWriteEarlyDeltaGFloorNW w j)) atTop (𝓝 0) := by
  ...
  have hsum :=
    (hdecay1.const_mul (Fintype.card UniversalLocalView : ℝ)).add
      (hdecay2.const_mul ((2 : ℝ) ^ (bgpHeadlineM + 1)))
...
theorem bgpNextWriteEarly_loser_mass_NW (w j : ℕ) :
  ...
  have hgap0 : 0 < gap := by
    simpa [gap] using
      solMURepl_static_hgap0 bgpHeadlineEta bgpHeadlineEta_pos
        bgpHeadlineHerr
  have hgap1516 : (15 : ℝ) / 16 ≤ gap := by
    have h := bgpRecoveryGap_ge_fifteen_sixteen w j
    simpa [bgpRecoveryGap, gap] using h
  ...
  have hKt0 : 0 ≤ Kt := by
    ...
    exact mul_nonneg (Real.exp_pos _).le
      (mul_nonneg (pow_nonneg (by nlinarith [Real.neg_one_le_cos s]) _)
        (by norm_num [bgpHeadlineKappa]))
  have hutube := bgpSegC_edge_tube_NW w j
  have hfloor := bgpNW4_floor_on_selToZOff w w j hutube
  ...
  have h1 := loser_mass_small_on_settled_window sol
    ...
    (fun s _hs => by
      exact mul_nonneg (pow_nonneg (by nlinarith [Real.neg_one_le_sin s]) _)
        (mul_nonneg (bgpWarmGainQNW_nonneg_real w) (Real.exp_pos _).le))
    (fun v hv s hs => by
      have h := bgpNW4_gap_of_tube w w j hutube v hv s
    ...
    (fun v hv => by
      have hfloor_a : 1 / (Fintype.card UniversalLocalView : ℝ) ≤
        ...
        exact hfloor a
          ⟨le_rfl, bgpNW4_sel_le_zOffStart j⟩
  ...
  have hcard1 : (0 : ℝ) ≤ (Fintype.card UniversalLocalView : ℝ) - 1 := by
    ...
    linarith
  have hΔG := bgpNextWriteEarlyDeltaGFloorNW_le w j ht.1
  have hΔGf0 := bgpNextWriteEarlyDeltaGFloorNW_nonneg w j
  have hXle : Real.exp (-(gap * (sol.G t - sol.G a))) ≤
      Real.exp (-(15 / 16) * bgpNextWriteEarlyDeltaGFloorNW w j) := by
    apply Real.exp_le_exp.mpr
  ...
  have hKtX : Kt * Real.exp (-(gap * (sol.G t - sol.G a))) ≤
    ...
    have hkill := bgpNW4_reset_kill w w j (t := t) (gap := gap)
      hat
      (le_trans ht.2
        (le_trans (selectorMUWriteHold_le_read j)
          (bgpNW4_wr_le_zOffStart j))) hgap1516
  ...
  calc
    ...
    _ ≤ ((Fintype.card UniversalLocalView : ℝ) - 1) *
      ...
      linarith
    _ = bgpNextWriteEarlyEpsNW w j := by
      rw [bgpNextWriteEarlyEpsNW]

/-! ## Weighted active-write contraction -/

/-- Z-gate mass on the late quarter of the active write. -/
noncomputable def bgpNextWriteEarlyMassLowerNW (w j : ℕ) : ℝ :=
  (Real.pi / 4) *
    Real.exp (50 * (bgpScaleW w : ℝ) *
      (2 * Real.pi * (j : ℝ) + Real.pi / 4))

theorem bgpNextWriteEarlyMassLowerNW_le_integral (w j : ℕ) :
    bgpNextWriteEarlyMassLowerNW w j ≤
      ∫ τ in (selectorMUEarlyWriteSubStart j)..(selectorMUWriteHoldTime j),
        (bgpParamsNW w).A * ((bgpHeadlineSolFamNW w) w).α τ *
          bGateZ (bgpParamsNW w).L
            (((bgpHeadlineSolFamNW w) w).μ τ) τ := by
    ...
    calc
      Real.exp (50 * (bgpScaleW w : ℝ) *
          ...
          ≤ Real.exp (50 * (bgpScaleW w : ℝ) * τ) := by
            ...
            nlinarith
      _ ≤ _ := bgpNW4_zKernel_ge_of_sin_ge_half w w hτ0 hsin
...
theorem bgpNextWriteEarlyMassLowerNW_tendsto_atTop (w : ℕ) :
    Tendsto (bgpNextWriteEarlyMassLowerNW w) atTop atTop := by
  have hlin : Tendsto
...
noncomputable def bgpNextWriteWeightedMixNW (w j : ℕ) : ℝ :=
  Real.exp (-(bgpNextWriteEarlyMassLowerNW w j)) +
    (Fintype.card UniversalLocalView : ℝ) *
      bgpNextWriteEarlyEpsNW w j +
    (Fintype.card UniversalLocalView : ℝ) * bgpNW4EpsLam w j

theorem bgpNextWriteWeightedMixNW_nonneg (w j : ℕ) :
    0 ≤ bgpNextWriteWeightedMixNW w j := by
  unfold bgpNextWriteWeightedMixNW
  exact add_nonneg
    (add_nonneg (Real.exp_nonneg _)
      (mul_nonneg (Nat.cast_nonneg _)
        (bgpNextWriteEarlyEpsNW_nonneg w j)))
    (mul_nonneg (Nat.cast_nonneg _) (bgpNW4EpsLam_nonneg w j))

theorem bgpNextWriteWeightedMixNW_tendsto_zero (w : ℕ) :
    Tendsto (bgpNextWriteWeightedMixNW w) atTop (𝓝 0) := by
  have hmass := Real.tendsto_exp_neg_atTop_nhds_zero.comp
    (bgpNextWriteEarlyMassLowerNW_tendsto_atTop w)
  have hearly := (bgpNextWriteEarlyEpsNW_tendsto_zero w).const_mul
    (Fintype.card UniversalLocalView : ℝ)
  have hhold := (bgpNW4EpsLam_tendsto_zero w).const_mul
    (Fintype.card UniversalLocalView : ℝ)
  change Tendsto
    (fun j => Real.exp (-(bgpNextWriteEarlyMassLowerNW w j)) +
      (Fintype.card UniversalLocalView : ℝ) *
        bgpNextWriteEarlyEpsNW w j +
      (Fintype.card UniversalLocalView : ℝ) * bgpNW4EpsLam w j)
    atTop (𝓝 0)
...
theorem bgpNextWrite_weighted_mix_NW (w j : ℕ) :
    (∫ τ in (selectorMUWriteStartTime j)..(selectorMUWriteHoldTime j),
      Real.exp (-(∫ σ in τ..(selectorMUWriteHoldTime j),
        (bgpParamsNW w).A * ((bgpHeadlineSolFamNW w) w).α σ *
          bGateZ (bgpParamsNW w).L
            (((bgpHeadlineSolFamNW w) w).μ σ) σ)) *
      ((bgpParamsNW w).A * ((bgpHeadlineSolFamNW w) w).α τ *
        bGateZ (bgpParamsNW w).L
      ...
      bgpNextWriteWeightedMixNW w j := by
  classical
  let sol := (bgpHeadlineSolFamNW w) w
  ...
  have hk_cont : Continuous k := by
    simpa [k, sol] using bgpNW4_zKernel_continuous w w
  ...
  have hk_nonneg : ∀ t ∈ Icc W H, 0 ≤ k t := by
    ...
    simpa [k, sol] using bgpNW4_zKernel_nonneg w w ht0
  have hH0 : 0 ≤ H := by
  ...
  have hprefix : ∀ t ∈ Icc W E, |d t| ≤ (1 : ℝ) := by
    ...
    have ht0 : 0 ≤ t := le_trans (selectorMUWriteStartTime_nonneg j)
      (by simpa [W] using ht.1)
    have hmT := (bgpNextWriteBoxInputsNW w).halt_mixTarget_mem_Icc w t ht0
    have hmH := (bgpNextWriteBoxInputsNW w).halt_mixTarget_mem_Icc w H hH0
    exact bgp_abs_sub_le_one_of_unit_interval_pair hmT hmH
  have hsuffix : ∀ t ∈ Icc E H, |d t| ≤ C := by
    intro t ht
    have ht0 : 0 ≤ t := le_trans (selectorMUWriteStartTime_nonneg j)
      (le_trans (selectorMUWriteStart_le_earlySubStart j)
        (by simpa [E] using ht.1))
    have hsumT := bgpNW_lam_sum_forward w w t ht0
    have hsumH := bgpNW_lam_sum_forward w w H hH0
    have hloserT := bgpNextWriteEarly_loser_mass_NW w j t
      (by simpa [E, H] using ht)
    have hloserH := bgpNW4_loser_mass_hold_window w j H
    ...
    have h := selectorMixTarget_halt_pair_of_loser_sum
      ...
      (fun v => bgpNW_lam_nonneg_forward w w v H hH0)
      hloserT hloserH
    simpa [d, C] using h
  have hC : 0 ≤ C := by
    exact add_nonneg
  ...
  have hmass := bgpNextWriteEarlyMassLowerNW_le_integral w j
  have hexp : Real.exp (-(∫ s in E..H, k s)) ≤
      Real.exp (-(bgpNextWriteEarlyMassLowerNW w j)) := by
    apply Real.exp_le_exp.mpr
  ...
  calc
    ...
    _ ≤ Real.exp (-(bgpNextWriteEarlyMassLowerNW w j)) + C :=
      add_le_add hexp le_rfl
    _ = bgpNextWriteWeightedMixNW w j := by
      unfold bgpNextWriteWeightedMixNW
      dsimp [C]
      ring
...
`cfg (j+1)`. -/
noncomputable def bgpNextWriteStartDeltaNW (w j : ℕ) : ℝ :=
  Real.exp (-(bgpNextWriteEarlyMassLowerNW w j)) +
    bgpNextWriteWeightedMixNW w j + bgpNW4EpsLam w j

theorem bgpNextWriteStartDeltaNW_nonneg (w j : ℕ) :
    0 ≤ bgpNextWriteStartDeltaNW w j := by
  unfold bgpNextWriteStartDeltaNW
  exact add_nonneg
    (add_nonneg (Real.exp_nonneg _)
      (bgpNextWriteWeightedMixNW_nonneg w j))
    (bgpNW4EpsLam_nonneg w j)

theorem bgpNextWriteStartDeltaNW_tendsto_zero (w : ℕ) :
    Tendsto (bgpNextWriteStartDeltaNW w) atTop (𝓝 0) := by
  have hmass := Real.tendsto_exp_neg_atTop_nhds_zero.comp
    (bgpNextWriteEarlyMassLowerNW_tendsto_atTop w)
  have hweighted := bgpNextWriteWeightedMixNW_tendsto_zero w
  have hhold := bgpNW4EpsLam_tendsto_zero w
  change Tendsto
    (fun j => Real.exp (-(bgpNextWriteEarlyMassLowerNW w j)) +
      bgpNextWriteWeightedMixNW w j + bgpNW4EpsLam w j)
    atTop (𝓝 0)
...
theorem bgpNextWrite_start_NW (w j : ℕ) :
    |((bgpHeadlineSolFamNW w) w).z (selectorMUWriteHoldTime j) haltCoordU -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤
        bgpNextWriteStartDeltaNW w j := by
  classical
  let sol := (bgpHeadlineSolFamNW w) w
  let W := selectorMUWriteStartTime j
  let E := selectorMUEarlyWriteSubStart j
  ...
  have hentry : |sol.z W haltCoordU - M| ≤ (1 : ℝ) := by
    have hz := (bgpNextWriteBoxInputsNW w).halt_z_mem_Icc
      (by rw [bgpParamsNW_A_eq]; norm_num : 0 ≤ (bgpParamsNW w).A)
      w W hW0
    have hmH := (bgpNextWriteBoxInputsNW w).halt_mixTarget_mem_Icc w H hH0
    exact bgp_abs_sub_le_one_of_unit_interval_pair hz hmH
  have hWE : W ≤ E := by
    simpa [W, E] using selectorMUWriteStart_le_earlySubStart j
  have hEH : E ≤ H := by
    simpa [E, H] using selectorMUEarlySubStart_le_writeHold j
  ...
  have hearlyMass := bgpNextWriteEarlyMassLowerNW_le_integral w j
  have hfullMass : bgpNextWriteEarlyMassLowerNW w j ≤
      ∫ t in W..H, k t := by
    have hearlyMass' : bgpNextWriteEarlyMassLowerNW w j ≤
        ∫ t in E..H, k t := by
  ...
  have hweighted :
    ...
    simpa [W, H, k, m, M, sol] using
      bgpNextWrite_weighted_mix_NW w j
  have hzM : |sol.z H haltCoordU - M| ≤
      Real.exp (-(bgpNextWriteEarlyMassLowerNW w j)) +
        bgpNextWriteWeightedMixNW w j := by
    exact hscalar.trans (add_le_add hctr hweighted)
  have hloser := bgpNW4_loser_mass_hold_window w j H
    (by simp [H, selectorMUWriteHold_le_read])
  have hmix : |m H -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU| ≤
        bgpNW4EpsLam w j := by
    have hraw := selectorMixTarget_halt_to_next_of_loser_sum_sharp
      ...
      (bgpNW_lam_sum_forward w w H hH0)
      (fun v => bgpNW_lam_nonneg_forward w w v H hH0) hloser
    simpa [m, solMUReplStaticCfg_step w j] using hraw
  calc
    |((bgpHeadlineSolFamNW w) w).z (selectorMUWriteHoldTime j)
        haltCoordU -
      stackMachineEncodingU.enc (solMUReplStaticCfg w (j + 1)) haltCoordU|
    ...
    _ ≤ (Real.exp (-(bgpNextWriteEarlyMassLowerNW w j)) +
          bgpNextWriteWeightedMixNW w j) + bgpNW4EpsLam w j :=
      add_le_add hzM hmix
    _ = bgpNextWriteStartDeltaNW w j := by
      rfl

/-- Final per-word next-write radius, shifted to cycle `j+1`. -/
noncomputable def bgpHeadlineNextWriteDeltaNW (w j : ℕ) : ℝ :=
  bgpNextWriteStartDeltaNW w (j + 1) + bgpNW4EpsLam w (j + 1)

theorem bgpHeadlineNextWriteDeltaNW_nonneg (w j : ℕ) :
    0 ≤ bgpHeadlineNextWriteDeltaNW w j := by
  unfold bgpHeadlineNextWriteDeltaNW
  exact add_nonneg (bgpNextWriteStartDeltaNW_nonneg w (j + 1))
    (bgpNW4EpsLam_nonneg w (j + 1))

theorem bgpHeadlineNextWriteDeltaNW_tendsto_zero (w : ℕ) :
    Tendsto (bgpHeadlineNextWriteDeltaNW w) atTop (𝓝 0) := by
  have hstart := (bgpNextWriteStartDeltaNW_tendsto_zero w).comp
    (Filter.tendsto_add_atTop_nat 1)
  have hmix := (bgpNW4EpsLam_tendsto_zero w).comp
    (Filter.tendsto_add_atTop_nat 1)
  change Tendsto
    (fun j => bgpNextWriteStartDeltaNW w (j + 1) +
      bgpNW4EpsLam w (j + 1)) atTop (𝓝 0)
...
theorem bgpHeadlineNextWrite_bound_NW (w j : ℕ) :
    ∀ t ∈ Icc (selectorMUNextWriteStart j) (selectorMUNextRead j),
      |((bgpHeadlineSolFamNW w) w).z t haltCoordU -
        ...
        bgpHeadlineNextWriteDeltaNW w j := by
  classical
  let n := j + 1
  let sol := (bgpHeadlineSolFamNW w) w
  ...
  have hg_cont : Continuous fun τ =>
    ...
    simpa [sol] using bgpNW4_zKernel_continuous w w
  have hg0 : ∀ τ ∈ Icc H t,
      0 ≤ (bgpParamsNW w).A * sol.α τ *
        bGateZ (bgpParamsNW w).L (sol.μ τ) τ := by
    intro τ hτ
    simpa [sol] using bgpNW4_zKernel_nonneg w w
      (le_trans hH0 hτ.1)
  have hmix : ∀ τ ∈ Icc H t,
    ...
    have hloser := bgpNW4_loser_mass_hold_window w n τ hτfull
    have hraw := selectorMixTarget_halt_to_next_of_loser_sum_sharp
      sol.u sol.lam τ (solMUReplStaticCfg w n)
      (bgpNW_lam_sum_forward w w τ hτ0)
      (fun v => bgpNW_lam_nonneg_forward w w v τ hτ0) hloser
  ...
  have hstart : |sol.z H haltCoordU - target| ≤
      bgpNextWriteStartDeltaNW w n := by
    simpa [sol, H, target, Nat.add_assoc] using
      bgpNextWrite_start_NW w n
...
def bgpHeadlineNextWriteNW (w : ℕ) :
  ...
  exact ⟨bgpHeadlineNextWriteDeltaNW w,
    bgpHeadlineNextWriteDeltaNW_tendsto_zero w,
    bgpHeadlineNextWriteDeltaNW_nonneg w,
    bgpHeadlineNextWrite_bound_NW w⟩
