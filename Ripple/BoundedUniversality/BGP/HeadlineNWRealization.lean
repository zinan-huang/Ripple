...
-- `paper3_warm_stereo_sum_sq` and `paper3_warm_stereo_abs_le_one` are now public
...
theorem main_assembled_dyn_selector_zreadout_nolatch_repl_W_explicit_pF
  ...
  · refine ⟨1, by norm_num, ?_⟩
    intro w τ i hτ
    exact paper3_warm_stereo_abs_le_one _ _
...
(`paper3HeadlineCorrectOfLateStart`-NW).  Assembling early surfaces any
keystone mismatch now, per the campaign doctrine. -/

/-- Rational value of the NW `cμ` initial coordinate. -/
def paper3NWCmuInitQ (w : ℕ) : ℚ := ((1000 * bgpScaleW w : ℕ) : ℚ)

/-- Rational value of the NW `cα` initial coordinate. -/
def paper3NWCalphaInitQ (w : ℕ) : ℚ := ((300 * bgpScaleW w : ℕ) : ℚ)

theorem paper3_bgpParamsNW_cμ_rat (w : ℕ) :
    (bgpParamsNW w).cμ = ((paper3NWCmuInitQ w : ℚ) : ℝ) := by
  rw [bgpParamsNW_cμ_def, paper3NWCmuInitQ]
  push_cast
  ring

theorem paper3_bgpParamsNW_cα_rat (w : ℕ) :
    (bgpParamsNW w).cα = ((paper3NWCalphaInitQ w : ℚ) : ℝ) := by
  rw [bgpParamsNW_cα_def, paper3NWCalphaInitQ]
  push_cast
  ring
...
private theorem paper3NW_zero_eq_rat_zero : (0 : ℝ) = ((0 : ℚ) : ℝ) := by
  norm_num

private theorem paper3NW_selectorSchedule_domain_nonneg :
...
noncomputable def paper3HeadlineFieldCC :
  ...
  selectorReplicatorAssembledFieldCC d_U B_U UniversalLocalView branchU
    (selChiResetPoly d_U UniversalLocalView paper3HeadlineM)
    (selChiGatePoly d_U UniversalLocalView paper3HeadlineM)
    (selKappaPoly d_U UniversalLocalView paper3HeadlineKappa)
    (selGainPoly d_U UniversalLocalView)
    (muReadoutPoly paper3HeadlineEta paper3HeadlineEta_pos)
    (0 : MvPolynomial (Fin d_U) ℚ)
    (1 : ℚ) (0 : ℚ) 1 paper3HeadlineR
...
theorem paper3_headline_unconditional_of_NW_readout_explicit
    (init_presented :
  ...
  refine main_assembled_dyn_selector_zreadout_nolatch_repl_W_explicit_pF
    ...
    paper3HeadlineFieldCC
    (fun w sol La => selectorReplicatorTupleTrajCC sol La
      ((paper3WarmGainQNW w : ℚ) : ℝ)
      ((paper3NWCmuInitQ w : ℚ) : ℝ) ((paper3NWCalphaInitQ w : ℚ) : ℝ))
    ?_
    (fun w => selectorReplicatorSphereInitQCC d_U UniversalLocalView selectorInitX0 w
      (paper3WarmGainQNW w) (paper3NWCmuInitQ w) (paper3NWCalphaInitQ w))
    init_presented haltCoordU (selEmbCC (selZ UniversalLocalView haltCoordU)) ?_
    (fun w => (paper3HeadlineSolFamNW w) w)
    (fun w => selector_replicator_zero_latch_solution ((paper3HeadlineSolFamNW w) w)
      paper3HeadlineR)
    hhalt hnonhalt ?_ ?_
  · intro w sol La t ht
    exact selectorReplicatorTupleTrajCC_ode sol La ((paper3WarmGainQNW w : ℚ) : ℝ)
      (selChiResetPoly d_U UniversalLocalView paper3HeadlineM)
      (selChiGatePoly d_U UniversalLocalView paper3HeadlineM)
      (selKappaPoly d_U UniversalLocalView paper3HeadlineKappa)
      (selGainPoly d_U UniversalLocalView)
      (muReadoutPoly paper3HeadlineEta paper3HeadlineEta_pos)
      (0 : MvPolynomial (Fin d_U) ℚ)
      (bgpParamsNW_A_eq_ratCast w) paper3NW_zero_eq_rat_zero
      (paper3_bgpParamsNW_cμ_rat w) (paper3_bgpParamsNW_cα_rat w)
      (bgpParamsNW_L_eq w)
      paper3NW_selectorSchedule_domain_nonneg
      (fun t _ht =>
        eval_selChiResetPoly_repl (sol := sol) (La := La)
          (warmGainVal := ((paper3WarmGainQNW w : ℚ) : ℝ)) (t := t) paper3HeadlineM)
      (fun t _ht =>
        eval_selChiGatePoly_repl (sol := sol) (La := La)
          (warmGainVal := ((paper3WarmGainQNW w : ℚ) : ℝ)) (t := t) paper3HeadlineM)
      (fun t _ht =>
        eval_selKappaPoly_repl (sol := sol) (La := La)
          (warmGainVal := ((paper3WarmGainQNW w : ℚ) : ℝ)) (t := t) paper3HeadlineKappa)
      (fun t ht => by
        rw [eval_selGainPoly_repl]
        rw [sol.alpha_eq_exp paper3NW_selectorSchedule_domain_nonneg ht])
      (fun v t _ht =>
        eval_muReadoutPoly_repl paper3HeadlineEta paper3HeadlineEta_pos sol La
          ((paper3WarmGainQNW w : ℚ) : ℝ) t v)
      (fun _t => by simp)
      t ht
  · intro w s La t
    simp only [selectorReplicatorTupleTrajCC_castAdd, selectorReplicatorTupleTraj_z]
  · intro w
    obtain ⟨hz0, hu0, hlam0, hG0⟩ := paper3HeadlineSolFamNW_initial_values w w
    exact selector_replicator_init_zero_of_initial_values_CC selectorInitX0 w
      (paper3WarmGainQNW w) (paper3NWCmuInitQ w) (paper3NWCalphaInitQ w)
      ((paper3HeadlineSolFamNW w) w)
      (selector_replicator_zero_latch_solution ((paper3HeadlineSolFamNW w) w)
        paper3HeadlineR) hz0 hu0 hlam0 hG0
  · intro w i
    obtain ⟨hz0, hu0, hlam0, hG0⟩ := paper3HeadlineSolFamNW_initial_values w w
    exact selector_replicator_init_succ_of_initial_values_CC selectorInitX0 w
      ...
      ((paper3HeadlineSolFamNW w) w)
      (selector_replicator_zero_latch_solution ((paper3HeadlineSolFamNW w) w)
        paper3HeadlineR) hz0 hu0 hlam0 hG0 i
