...
/-- Compatibility wrapper for Mathlib's `geometricMeasure`.

As of the Mathlib `v4.30.0` API, `ProbabilityTheory.geometricMeasure` takes a
bundled `unitInterval` parameter `p`.  This file is written against the earlier
unbundled interface `geometricMeasure' (hp_pos : 0 < p) (hp_le_one : p ≤ 1)`, so
we package the two membership hypotheses into the `unitInterval` element here. -/
noncomputable def geometricMeasure' {p : ℝ} (hp_pos : 0 < p) (hp_le_one : p ≤ 1) :
    Measure ℕ :=
  ProbabilityTheory.geometricMeasure ⟨p, ⟨hp_pos.le, hp_le_one⟩⟩

/-- The compatibility wrapper coincides with the `PMF`-induced measure of the
geometric `PMF`.  Both are countable measures on `ℕ` putting mass
`ENNReal.ofReal ((1 - p)^n * p)` on the singleton `{n}`. -/
theorem geometricMeasure'_eq_toMeasure {p : ℝ} (hp_pos : 0 < p) (hp_le_one : p ≤ 1) :
    geometricMeasure' hp_pos hp_le_one
      = (ProbabilityTheory.geometricPMF hp_pos hp_le_one).toMeasure := by
  have hp0 : (⟨p, ⟨hp_pos.le, hp_le_one⟩⟩ : unitInterval) ≠ 0 := by
    intro h
    have : p = 0 := congrArg Subtype.val h
    exact (lt_irrefl (0 : ℝ)) (this ▸ hp_pos)
  ext s hs
  rw [(geometricPMF hp_pos hp_le_one).toMeasure_apply hs]
  rw [geometricMeasure', ProbabilityTheory.geometricMeasure_eq hp0,
    Measure.sum_apply _ hs]
  refine tsum_congr fun n => ?_
  rw [Measure.smul_apply, Measure.dirac_apply' _ hs, smul_eq_mul]
  by_cases hn : n ∈ s
  · simp only [Set.indicator_of_mem hn, Pi.one_apply, mul_one]
    show ENNReal.ofReal ((1 - p) ^ n * p)
      = ENNReal.ofReal (ProbabilityTheory.geometricPMFReal p n)
...
theorem shifted_geometric_exp_integral_geometricMeasure {p t : ℝ}
    ...
    (∫ n : ℕ, Real.exp (t * ((n : ℝ) + 1)) ∂geometricMeasure' hp_pos hp_le_one) =
      Real.exp t * p * (1 - (1 - p) * Real.exp t)⁻¹ := by
  rw [geometricMeasure'_eq_toMeasure hp_pos hp_le_one]
