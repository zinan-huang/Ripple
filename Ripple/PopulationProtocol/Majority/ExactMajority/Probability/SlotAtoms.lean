...
* **#15 — `work0` / `work2` / `work3` / `work9` (carried opaque `PhaseConvergenceW` instances).**
  ...
  `work3 ← Phase3Post3.slot3PostWork` / `Phase3Assembly.slot3_of_entry`, `work2`/`work9 ←
...
`#print axioms ⊆ {propext, Classical.choice, Quot.sound}`; no proof placeholders or eval shortcuts.
-/
...
* slot 3 — §103/Post₃ endpoint (`Phase3Post3.slot3PostWork`);
* slot 9 — pre-phase-10 opinion-union (`phase9ConvergenceW`).
...
/-- **slot 3 — direct §103/Post₃ work.**  This bypasses the old
`EndpointWiring.phase3Convergence_bounded` `Sgood` side abstraction. -/
noncomputable def slot3W {n ell : ℕ} {M g₀ : ℝ} {σ : Sign}
    (A : Phase3Post3.Slot3PostTail (L := L) (K := K) n ell M g₀ σ) :
    PhaseConvergenceW (NonuniformMajority L K).transitionKernel :=
  Phase3Post3.slot3PostWork (L := L) (K := K) A

/-- **slot 3 — protocol-entry §6/Post₃ work.**  This is the honest entry-level
...
noncomputable def slot3W_of_entry
    ...
    (A : Phase3Assembly.Slot3OfEntryResiduals
      (L := L) (K := K) D θ n ell M g₀ σ) :
    PhaseConvergenceW (NonuniformMajority L K).transitionKernel :=
  Phase3Assembly.slot3_of_entry (L := L) (K := K) A

/-- **slot 9 — the pre-phase-10 opinion-union instance.**  Same scalar/opinion surface as slot 2,
...
theorem slotInstances_of_named {n : ℕ} (si : SlotInstanceInputs (L := L) (K := K) n) :
    ...
    si.work3 = Phase3Post3.slot3PostWork (L := L) (K := K) si.s3post ∧
...
with the work `Post` ⟹ drained-state read.  This delivers the `Assembly'.hSeedStep` field. -/
