...
typecheck ignores all of those).  `theorem_3_1_fully_unconditional` already carries `T`/`hT`,
...
/-! ## Part 5 — `theorem_3_1_fully_unconditional`: the headline, `hReach10`-FREE.
...
set_option maxHeartbeats 2000000 in
theorem theorem_3_1_fully_unconditional {n : ℕ}
...
`reach10_chain_restricted`).  `theorem_3_1_fully_unconditional` therefore takes the residual
...
`FaithfulCore`-conditioned `FaithfulWitness.theorem_3_1_unconditional`: the false field is gone.
-/
...
#print axioms theorem_3_1_fully_unconditional
