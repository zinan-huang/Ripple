...
phase 0, role MCR, with `assigned = false`, and the smallBias initialized
from the input opinion (A → +1 = Fin 4, B → −1 = Fin 2). -/
def validInitial {L K : ℕ} (c : Config (AgentState L K)) : Prop :=
  ∀ a ∈ c, a.phase = ⟨0, by decide⟩ ∧ a.role = .mcr ∧ a.assigned = false ∧
...
/-- Target statement for **stable correctness** (Doty Theorem 3.1,
correctness part). For every
valid initial configuration, every reachable configuration can reach a
stable configuration whose output equals the majority verdict.

This is intentionally an `abbrev`, not a theorem: the §§5-7 analysis has not
yet been connected to the generic `StablyComputes` definition. -/
abbrev stable_majority_correct_target (L K : ℕ) : Prop :=
...
/-- Target statement for the correctness part of **Doty et al. Theorem 3.1**.
The proven exported theorem in this file is currently `state_count_poly_bound`;
this target records the remaining stable-computation obligation without
pretending it has been proved. -/
abbrev nonuniform_majority_correctness_target (L K : ℕ) : Prop :=
  stable_majority_correct_target L K
