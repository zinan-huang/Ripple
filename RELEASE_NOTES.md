...
The project has **no `sorry`** and **no named `axiom` declarations**.
`#print axioms` on every headline result reports only Lean's three
standard axioms (`propext`, `Classical.choice`, `Quot.sound`). The only
