...
\begin{abstract}
The \emph{selective dual-railing} construction of Huang--Klinge--Lathrop~\cite{UCNC25}
...
\item \textbf{Quintic case.}  The next polynomial in the sequence is
      ...
      $\hat{p}^{-} = u^{5} + 10 u^{3} v^{2} + 5 u v^{4}$.  The $\sigma$-drift
      is a quintic rather than a cubic in~$\sigma$, and the discriminant
      structure is more complicated (it is a sextic in~$k$).  We expect a
      similar saddle-node threshold but have not identified a closed-form
      barrier height.
...
\newblock Selective dual-railing for chemical reaction network simulation.
